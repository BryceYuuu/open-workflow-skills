...
description: "Collect saved URLs, extract readable content, rank relevance, and produce scheduled daily or weekly digests. Use when the user asks for read later digest or an equivalent workflow."
license: MIT
compatibility: "Cross-agent Agent Skills format. Core stack: Reader/browser / storage / scheduler. Check runtime dependencies before execution."
metadata:
  ...
  search-keywords: "read-later-digest, Read Later Digest, 稍后阅读摘要, read later summary, article digest, daily reading digest, 稍后阅读, 文章摘要, personal productivity, AI assistant, personal automation, 个人效率, AI助手, 个人自动化"
  quality-score: "4.0/5"
---
# Read Later Digest / 稍后阅读摘要

## Purpose / 用途

**中文**

读取保存的网页链接，提取正文、排序重要性并生成每日或每周摘要。

**English**

Collect saved URLs, extract readable content, rank relevance, and produce scheduled daily or weekly digests.
...
`read-later-digest` · `Read Later Digest` · `稍后阅读摘要` · `read later summary` · `article digest` · `daily reading digest` · `稍后阅读` · `文章摘要` · `personal productivity` · `AI assistant` · `personal automation` · `个人效率` · `AI助手` · `个人自动化`

## When to use / 何时使用

Use this skill when the user explicitly requests **read later digest**, or when the requested outcome clearly matches this workflow.
...
`Reader/browser / storage / scheduler`
...
```bash
npx skills add BryceYuuu/open-workflow-skills --skill read-later-digest
```

### Install for a specific supported agent

```bash
npx skills add BryceYuuu/open-workflow-skills --skill read-later-digest --agent codex
```
...
> 使用 `read-later-digest` 工作流处理我提供的输入。先检查依赖和权限，不覆盖源文件；执行后给我输出路径、验证结果和需要人工审核的部分。

**English**

> Use the `read-later-digest` workflow for the provided input. Check dependencies and permissions first, preserve the source, validate the result, and report outputs plus any human-review items.
