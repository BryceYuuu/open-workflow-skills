---
name: expense-parser
description: "Extract merchant, date, amount, tax, and category from receipts and map them to an expense schema. Use when the user asks for expense parser or an equivalent workflow."
license: MIT
...
metadata:
  ...
  quality-score: "4.5/5"
---
# Expense Parser / 报销票据识别

## Purpose / 用途

**中文**

识别发票和收据中的商户、日期、金额、税额和类别，并映射到报销字段；正式提交需按企业规则配置。

**English**

Extract merchant, date, amount, tax, and category from receipts and map them to an expense schema.
...
`expense-parser` · `Expense Parser` · `报销票据识别` · `receipt OCR` · `expense automation` · `invoice OCR` · `报销自动化` · `发票识别` · `office automation` · `AI productivity` · `workflow automation` · `办公自动化` · `效率工具` · `AI办公`

## When to use / 何时使用

Use this skill when the user explicitly requests **expense parser**, or when the requested outcome clearly matches this workflow.
...
`Vision/OCR / expense app optional`
...
```bash
npx skills add BryceYuuu/open-workflow-skills --skill expense-parser
```

### Install for a specific supported agent

```bash
npx skills add BryceYuuu/open-workflow-skills --skill expense-parser --agent codex
```
...
> 使用 `expense-parser` 工作流处理我提供的输入。先检查依赖和权限，不覆盖源文件；执行后给我输出路径、验证结果和需要人工审核的部分。

**English**

> Use the `expense-parser` workflow for the provided input. Check dependencies and permissions first, preserve the source, validate the result, and report outputs plus any human-review items.
