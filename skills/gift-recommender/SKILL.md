...
description: "Recommend gift categories or products from relationship, interests, occasion, budget, and live availability when connected. Use when the user asks for gift recommender or an equivalent workflow."
license: MIT
compatibility: "Cross-agent Agent Skills format. Core stack: Search/product data optional. Check runtime dependencies before execution."
metadata:
  ...
  status: "experimental"
  automation-level: "L3"
  search-keywords: "gift-recommender, Gift Recommender, 礼物推荐, AI gift ideas, gift recommendation, 送礼建议, personal productivity, AI assistant, personal automation, 个人效率, AI助手, 个人自动化"
  quality-score: "3.5/5"
---
# Gift Recommender / 礼物推荐

## Purpose / 用途

**中文**

根据关系、兴趣、场景和预算推荐礼物；具体商品质量依赖地区、库存和实时价格。

**English**

Recommend gift categories or products from relationship, interests, occasion, budget, and live availability when connected.
...
`gift-recommender` · `Gift Recommender` · `礼物推荐` · `AI gift ideas` · `gift recommendation` · `送礼建议` · `personal productivity` · `AI assistant` · `personal automation` · `个人效率` · `AI助手` · `个人自动化`

## When to use / 何时使用

Use this skill when the user explicitly requests **gift recommender**, or when the requested outcome clearly matches this workflow.
...
```bash
npx skills add BryceYuuu/open-workflow-skills --skill gift-recommender
```

### Install for a specific supported agent

```bash
npx skills add BryceYuuu/open-workflow-skills --skill gift-recommender --agent codex
```
...
> 使用 `gift-recommender` 工作流处理我提供的输入。先检查依赖和权限，不覆盖源文件；执行后给我输出路径、验证结果和需要人工审核的部分。

**English**

> Use the `gift-recommender` workflow for the provided input. Check dependencies and permissions first, preserve the source, validate the result, and report outputs plus any human-review items.
