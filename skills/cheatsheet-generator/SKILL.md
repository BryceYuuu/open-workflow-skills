...
description: "Create concise version-aware reference sheets for technical subjects, commands, APIs, or operating procedures. Use when the user asks for cheatsheet generator or an equivalent workflow."
license: MIT
...
metadata:
  ...
  search-keywords: "cheatsheet-generator, Cheatsheet Generator, 一页速查表, AI cheatsheet, quick reference generator, 速查表, 技术速查, AI research, learning automation, education AI, 研究助手, 学习自动化, AI教育"
  quality-score: "4.5/5"
---
# Cheatsheet Generator / 一页速查表

## Purpose / 用途

**中文**

为技术主题、命令、API 或操作流程生成简洁且标明版本的一页速查表。

**English**

Create concise version-aware reference sheets for technical subjects, commands, APIs, or operating procedures.
...
`cheatsheet-generator` · `Cheatsheet Generator` · `一页速查表` · `AI cheatsheet` · `quick reference generator` · `速查表` · `技术速查` · `AI research` · `learning automation` · `education AI` · `研究助手` · `学习自动化` · `AI教育`

## When to use / 何时使用

Use this skill when the user explicitly requests **cheatsheet generator**, or when the requested outcome clearly matches this workflow.
...
```bash
npx skills add BryceYuuu/open-workflow-skills --skill cheatsheet-generator
```

### Install for a specific supported agent

```bash
npx skills add BryceYuuu/open-workflow-skills --skill cheatsheet-generator --agent codex
```
...
> 使用 `cheatsheet-generator` 工作流处理我提供的输入。先检查依赖和权限，不覆盖源文件；执行后给我输出路径、验证结果和需要人工审核的部分。

**English**

> Use the `cheatsheet-generator` workflow for the provided input. Check dependencies and permissions first, preserve the source, validate the result, and report outputs plus any human-review items.
