...
description: "Generate brand identity directions, palettes, typography proposals, tokens, and example applications from a brand brief; not a substitute for brand strategy research. Use when the user asks for brand kit generator or an equivalent workflow."
license: MIT
compatibility: "Cross-agent Agent Skills format. Core stack: LLM / image generation optional / design tokens. Check runtime dependencies before execution."
metadata:
  ...
  status: "experimental"
  automation-level: "L3"
  search-keywords: "brand-kit-generator, Brand Kit Generator, 品牌视觉方向生成, AI brand kit, brand identity generator, logo color typography, 品牌视觉, 品牌设计, AI design, design automation, creative automation, AI设计, 设计自动化, 创意工作流"
  quality-score: "3.0/5"
---
# Brand Kit Generator / 品牌视觉方向生成

## Purpose / 用途

**中文**

根据品牌 Brief 生成视觉方向、配色、字体、Token 和应用示例；不能替代完整品牌战略和用户研究。

**English**

Generate brand identity directions, palettes, typography proposals, tokens, and example applications from a brand brief; not a substitute for brand strategy research.
...
`brand-kit-generator` · `Brand Kit Generator` · `品牌视觉方向生成` · `AI brand kit` · `brand identity generator` · `logo color typography` · `品牌视觉` · `品牌设计` · `AI design` · `design automation` · `creative automation` · `AI设计` · `设计自动化` · `创意工作流`

## When to use / 何时使用

Use this skill when the user explicitly requests **brand kit generator**, or when the requested outcome clearly matches this workflow.
...
`LLM / image generation optional / design tokens`
...
```bash
npx skills add BryceYuuu/open-workflow-skills --skill brand-kit-generator
```

### Install for a specific supported agent

```bash
npx skills add BryceYuuu/open-workflow-skills --skill brand-kit-generator --agent codex
```
...
> 使用 `brand-kit-generator` 工作流处理我提供的输入。先检查依赖和权限，不覆盖源文件；执行后给我输出路径、验证结果和需要人工审核的部分。

**English**

> Use the `brand-kit-generator` workflow for the provided input. Check dependencies and permissions first, preserve the source, validate the result, and report outputs plus any human-review items.
