---
name: habit-planner
description: "Break a goal into minimum repeatable actions, schedule review cycles, and produce a lightweight tracking plan. Use when the user asks for habit planner or an equivalent workflow."
license: MIT
...
metadata:
  ...
  quality-score: "4.0/5"
---
# Habit Planner / 习惯计划生成

## Purpose / 用途

**中文**

把目标拆成最小可执行动作、复盘周期和轻量追踪计划。

**English**

Break a goal into minimum repeatable actions, schedule review cycles, and produce a lightweight tracking plan.
...
`habit-planner` · `Habit Planner` · `习惯计划生成` · `AI habit tracker` · `habit plan` · `21 day habit` · `习惯养成` · `打卡计划` · `personal productivity` · `AI assistant` · `personal automation` · `个人效率` · `AI助手` · `个人自动化`

## When to use / 何时使用

Use this skill when the user explicitly requests **habit planner**, or when the requested outcome clearly matches this workflow.
...
```bash
npx skills add BryceYuuu/open-workflow-skills --skill habit-planner
```

### Install for a specific supported agent

```bash
npx skills add BryceYuuu/open-workflow-skills --skill habit-planner --agent codex
```
...
> 使用 `habit-planner` 工作流处理我提供的输入。先检查依赖和权限，不覆盖源文件；执行后给我输出路径、验证结果和需要人工审核的部分。

**English**

> Use the `habit-planner` workflow for the provided input. Check dependencies and permissions first, preserve the source, validate the result, and report outputs plus any human-review items.
