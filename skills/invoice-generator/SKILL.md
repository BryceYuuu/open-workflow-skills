...
description: "Generate invoices from order or contract data, validate required fields, and render PDF or structured output; jurisdictional compliance is configurable. Use when the user asks for invoice generator or an equivalent workflow."
license: MIT
compatibility: "Cross-agent Agent Skills format. Core stack: Template engine / PDF / validation. Check runtime dependencies before execution."
metadata:
  ...
  search-keywords: "invoice-generator, Invoice Generator, 商业发票生成, invoice generator, billing automation, PDF invoice, 发票生成, 账单生成, business automation, growth automation, AI business, 商业自动化, 增长自动化, AI商业"
  quality-score: "4.5/5"
---
# Invoice Generator / 商业发票生成

## Purpose / 用途

**中文**

根据订单或合同数据生成发票、检查必填字段并输出 PDF/结构化数据；各地区合规要求需单独配置。

**English**

Generate invoices from order or contract data, validate required fields, and render PDF or structured output; jurisdictional compliance is configurable.
...
`invoice-generator` · `Invoice Generator` · `商业发票生成` · `invoice generator` · `billing automation` · `PDF invoice` · `发票生成` · `账单生成` · `business automation` · `growth automation` · `AI business` · `商业自动化` · `增长自动化` · `AI商业`

## When to use / 何时使用

Use this skill when the user explicitly requests **invoice generator**, or when the requested outcome clearly matches this workflow.
...
`Template engine / PDF / validation`
...
```bash
npx skills add BryceYuuu/open-workflow-skills --skill invoice-generator
```

### Install for a specific supported agent

```bash
npx skills add BryceYuuu/open-workflow-skills --skill invoice-generator --agent codex
```
...
> 使用 `invoice-generator` 工作流处理我提供的输入。先检查依赖和权限，不覆盖源文件；执行后给我输出路径、验证结果和需要人工审核的部分。

**English**

> Use the `invoice-generator` workflow for the provided input. Check dependencies and permissions first, preserve the source, validate the result, and report outputs plus any human-review items.
