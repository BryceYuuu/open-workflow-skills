---
name: font-pairing
description: "Recommend typography systems based on brand tone, language coverage, platform use, and license constraints. Use when the user asks for font pairing or an equivalent workflow."
license: MIT
...
根据品牌气质、语言覆盖、使用场景和字体授权约束推荐字体系统。

**English**

Recommend typography systems based on brand tone, language coverage, platform use, and license constraints.
...
`font-pairing` · `Font Pairing` · `字体搭配建议` · `font pairing AI` · `typography recommendations` · `字体搭配` · `字体推荐` · `AI design` · `design automation` · `creative automation` · `AI设计` · `设计自动化` · `创意工作流`

## When to use / 何时使用

Use this skill when the user explicitly requests **font pairing**, or when the requested outcome clearly matches this workflow.
...
```bash
npx skills add BryceYuuu/open-workflow-skills --skill font-pairing
```

### Install for a specific supported agent

```bash
npx skills add BryceYuuu/open-workflow-skills --skill font-pairing --agent codex
```
...
> 使用 `font-pairing` 工作流处理我提供的输入。先检查依赖和权限，不覆盖源文件；执行后给我输出路径、验证结果和需要人工审核的部分。

**English**

> Use the `font-pairing` workflow for the provided input. Check dependencies and permissions first, preserve the source, validate the result, and report outputs plus any human-review items.
