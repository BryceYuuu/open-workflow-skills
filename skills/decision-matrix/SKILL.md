...
description: "Build weighted decision matrices, calculate scores deterministically, and run sensitivity analysis over user-defined criteria. Use when the user asks for decision matrix or an equivalent workflow."
license: MIT
compatibility: "Cross-agent Agent Skills format. Core stack: Python/spreadsheet optional. Check runtime dependencies before execution."
metadata:
  ...
  search-keywords: "decision-matrix, Decision Matrix, 决策矩阵, AI decision matrix, weighted decision, decision analysis, 选择分析, personal productivity, AI assistant, personal automation, 个人效率, AI助手, 个人自动化"
  quality-score: "5.0/5"
---
...
根据用户定义的维度和权重建立决策矩阵，以确定性方式计算分数并做敏感性分析。

**English**

Build weighted decision matrices, calculate scores deterministically, and run sensitivity analysis over user-defined criteria.
...
`decision-matrix` · `Decision Matrix` · `决策矩阵` · `AI decision matrix` · `weighted decision` · `decision analysis` · `选择分析` · `personal productivity` · `AI assistant` · `personal automation` · `个人效率` · `AI助手` · `个人自动化`

## When to use / 何时使用

Use this skill when the user explicitly requests **decision matrix**, or when the requested outcome clearly matches this workflow.
...
```bash
npx skills add BryceYuuu/open-workflow-skills --skill decision-matrix
```

### Install for a specific supported agent

```bash
npx skills add BryceYuuu/open-workflow-skills --skill decision-matrix --agent codex
```
...
> 使用 `decision-matrix` 工作流处理我提供的输入。先检查依赖和权限，不覆盖源文件；执行后给我输出路径、验证结果和需要人工审核的部分。

**English**

> Use the `decision-matrix` workflow for the provided input. Check dependencies and permissions first, preserve the source, validate the result, and report outputs plus any human-review items.
