---
name: recording-to-content
description: "Repurpose a recording into transcript, article, newsletter, social posts, and short-video scripts. Use when the user asks for recording to content or an equivalent workflow."
license: MIT
...
metadata:
  ...
  quality-score: "4.5/5"
---
# Recording to Content / 录音多平台内容生成

## Purpose / 用途

**中文**

把一段录音同时转成文字稿、文章、Newsletter、社媒内容和短视频脚本。

**English**

Repurpose a recording into transcript, article, newsletter, social posts, and short-video scripts.
...
`recording-to-content` · `Recording to Content` · `录音多平台内容生成` · `audio to content` · `recording transcription` · `content repurposing` · `录音转文案` · `一稿多发` · `AI content automation` · `content automation` · `video automation` · `social media automation` · `内容自动化` · `视频自动化`

## When to use / 何时使用

Use this skill when the user explicitly requests **recording to content**, or when the requested outcome clearly matches this workflow.
...
```bash
npx skills add BryceYuuu/open-workflow-skills --skill recording-to-content
```

### Install for a specific supported agent

```bash
npx skills add BryceYuuu/open-workflow-skills --skill recording-to-content --agent codex
```
...
> 使用 `recording-to-content` 工作流处理我提供的输入。先检查依赖和权限，不覆盖源文件；执行后给我输出路径、验证结果和需要人工审核的部分。

**English**

> Use the `recording-to-content` workflow for the provided input. Check dependencies and permissions first, preserve the source, validate the result, and report outputs plus any human-review items.
