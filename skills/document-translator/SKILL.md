...
description: "Translate DOCX, PPTX, and supported PDFs while preserving structure where practical; layout fidelity varies by format. Use when the user asks for document translator or an equivalent workflow."
license: MIT
compatibility: "Cross-agent Agent Skills format. Core stack: Document parser / LLM / office libraries. Check runtime dependencies before execution."
metadata:
  ...
  search-keywords: "document-translator, Document Translator, 文档翻译与排版保留, document translation, PDF translator, Word translator, PPT translator, 文档翻译, PDF翻译, office automation, AI productivity, workflow automation, 办公自动化, 效率工具"
  quality-score: "3.0/5"
---
# Document Translator / 文档翻译与排版保留

## Purpose / 用途

**中文**

翻译 DOCX、PPTX 和可处理的 PDF，并尽量保留结构；复杂 PDF 的原排版无法保证百分百恢复。

**English**

Translate DOCX, PPTX, and supported PDFs while preserving structure where practical; layout fidelity varies by format.
...
`document-translator` · `Document Translator` · `文档翻译与排版保留` · `document translation` · `PDF translator` · `Word translator` · `PPT translator` · `文档翻译` · `PDF翻译` · `office automation` · `AI productivity` · `workflow automation` · `办公自动化` · `效率工具`

## When to use / 何时使用

Use this skill when the user explicitly requests **document translator**, or when the requested outcome clearly matches this workflow.
...
`Document parser / LLM / office libraries`
...
```bash
npx skills add BryceYuuu/open-workflow-skills --skill document-translator
```

### Install for a specific supported agent

```bash
npx skills add BryceYuuu/open-workflow-skills --skill document-translator --agent codex
```
...
> 使用 `document-translator` 工作流处理我提供的输入。先检查依赖和权限，不覆盖源文件；执行后给我输出路径、验证结果和需要人工审核的部分。

**English**

> Use the `document-translator` workflow for the provided input. Check dependencies and permissions first, preserve the source, validate the result, and report outputs plus any human-review items.
