...
description: "Classify sentiment and cluster recurring topics, pain points, feature requests, and customer insights from text datasets. Use when the user asks for sentiment analysis or an equivalent workflow."
license: MIT
compatibility: "Cross-agent Agent Skills format. Core stack: NLP / LLM / Python. Check runtime dependencies before execution."
metadata:
  ...
  search-keywords: "sentiment-analysis, Sentiment Analysis, 用户情绪与主题分析, sentiment analysis AI, review analysis, customer feedback analysis, 舆情分析, 评论情感分析, data automation, AI data analysis, Excel automation, analytics workflow, 数据自动化, 数据分析"
  quality-score: "3.5/5"
---
# Sentiment Analysis / 用户情绪与主题分析

## Purpose / 用途

**中文**

对评论、客服、问卷等文本进行情绪分类和主题聚类，识别痛点、需求和用户洞察。

**English**

Classify sentiment and cluster recurring topics, pain points, feature requests, and customer insights from text datasets.
...
`sentiment-analysis` · `Sentiment Analysis` · `用户情绪与主题分析` · `sentiment analysis AI` · `review analysis` · `customer feedback analysis` · `舆情分析` · `评论情感分析` · `data automation` · `AI data analysis` · `Excel automation` · `analytics workflow` · `数据自动化` · `数据分析`

## When to use / 何时使用

Use this skill when the user explicitly requests **sentiment analysis**, or when the requested outcome clearly matches this workflow.
...
```bash
npx skills add BryceYuuu/open-workflow-skills --skill sentiment-analysis
```

### Install for a specific supported agent

```bash
npx skills add BryceYuuu/open-workflow-skills --skill sentiment-analysis --agent codex
```
...
> 使用 `sentiment-analysis` 工作流处理我提供的输入。先检查依赖和权限，不覆盖源文件；执行后给我输出路径、验证结果和需要人工审核的部分。

**English**

> Use the `sentiment-analysis` workflow for the provided input. Check dependencies and permissions first, preserve the source, validate the result, and report outputs plus any human-review items.
