...
description: "Analyze spreadsheet schemas, propose KPIs, generate charts, and build a reproducible dashboard or report. Use when the user asks for spreadsheet dashboard or an equivalent workflow."
license: MIT
compatibility: "Cross-agent Agent Skills format. Core stack: Python / Pandas / plotting. Check runtime dependencies before execution."
metadata:
  ...
  automation-level: "L2"
  search-keywords: "spreadsheet-dashboard, Spreadsheet Dashboard, 表格分析与 Dashboard, Excel dashboard, AI spreadsheet analysis, CSV dashboard, Excel数据看板, 表格分析, data automation, AI data analysis, Excel automation, analytics workflow, 数据自动化, 数据分析"
  quality-score: "4.0/5"
---
# Spreadsheet Dashboard / 表格分析与 Dashboard

## Purpose / 用途

**中文**

分析 Excel/CSV 的字段和指标，建议 KPI、生成图表和可复现 Dashboard；业务 KPI 最好由用户确认。

**English**

Analyze spreadsheet schemas, propose KPIs, generate charts, and build a reproducible dashboard or report.
...
`spreadsheet-dashboard` · `Spreadsheet Dashboard` · `表格分析与 Dashboard` · `Excel dashboard` · `AI spreadsheet analysis` · `CSV dashboard` · `Excel数据看板` · `表格分析` · `data automation` · `AI data analysis` · `Excel automation` · `analytics workflow` · `数据自动化` · `数据分析`

## When to use / 何时使用

Use this skill when the user explicitly requests **spreadsheet dashboard**, or when the requested outcome clearly matches this workflow.
...
3. **Calculate row/column/missing-value statistics**
4. **Calculate min/max/mean for numeric fields**
5. **Render self-contained HTML dashboard**
6. **Write machine-readable JSON metrics**
...
```bash
ows doctor spreadsheet-dashboard
ows run spreadsheet-dashboard -- --help
ows test spreadsheet-dashboard
```
...
```bash
npx skills add BryceYuuu/open-workflow-skills --skill spreadsheet-dashboard
```

### Install for a specific supported agent

```bash
npx skills add BryceYuuu/open-workflow-skills --skill spreadsheet-dashboard --agent codex
```
...
> 使用 `spreadsheet-dashboard` 工作流处理我提供的输入。先检查依赖和权限，不覆盖源文件；执行后给我输出路径、验证结果和需要人工审核的部分。

**English**

> Use the `spreadsheet-dashboard` workflow for the provided input. Check dependencies and permissions first, preserve the source, validate the result, and report outputs plus any human-review items.
