---
name: weekly-report
description: "Collect metrics from connected sources and produce a structured weekly report with changes, explanations, risks, and next actions. Use when the user asks for weekly report or an equivalent workflow."
license: MIT
compatibility: "Cross-agent Agent Skills format. Core stack: Data connectors / scheduler / LLM. Check runtime dependencies before execution."
metadata:
  ...
  status: "integration"
  automation-level: "L4"
  search-keywords: "weekly-report, Weekly Report, 自动业务周报, AI weekly report, automatic business report, report automation, 自动周报, 业务周报, office automation, AI productivity, workflow automation, 办公自动化, 效率工具, AI办公"
  quality-score: "4.5/5"
---
# Weekly Report / 自动业务周报

## Purpose / 用途

**中文**

从已连接的数据源收集指标，生成变化、原因、风险和下一步行动组成的周报。

**English**

Collect metrics from connected sources and produce a structured weekly report with changes, explanations, risks, and next actions.
...
`weekly-report` · `Weekly Report` · `自动业务周报` · `AI weekly report` · `automatic business report` · `report automation` · `自动周报` · `业务周报` · `office automation` · `AI productivity` · `workflow automation` · `办公自动化` · `效率工具` · `AI办公`

## When to use / 何时使用

Use this skill when the user explicitly requests **weekly report**, or when the requested outcome clearly matches this workflow.
...
```bash
npx skills add BryceYuuu/open-workflow-skills --skill weekly-report
```

### Install for a specific supported agent

```bash
npx skills add BryceYuuu/open-workflow-skills --skill weekly-report --agent codex
```
...
> 使用 `weekly-report` 工作流处理我提供的输入。先检查依赖和权限，不覆盖源文件；执行后给我输出路径、验证结果和需要人工审核的部分。

**English**

> Use the `weekly-report` workflow for the provided input. Check dependencies and permissions first, preserve the source, validate the result, and report outputs plus any human-review items.
