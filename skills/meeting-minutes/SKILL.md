...
description: "Turn meeting audio or transcripts into decisions, action items, owners, deadlines, and unresolved questions. Use when the user asks for meeting minutes or an equivalent workflow."
license: MIT
...
metadata:
  ...
  search-keywords: "meeting-minutes, Meeting Minutes, 会议纪要与待办, AI meeting notes, meeting transcription, meeting summary, 会议纪要, 会议转文字, office automation, AI productivity, workflow automation, 办公自动化, 效率工具, AI办公"
  quality-score: "4.5/5"
---
# Meeting Minutes / 会议纪要与待办

## Purpose / 用途

**中文**

把会议录音或文字稿整理成结论、待办、负责人、Deadline 和未决问题；不得编造会议中未明确的信息。

**English**

Turn meeting audio or transcripts into decisions, action items, owners, deadlines, and unresolved questions.
...
`meeting-minutes` · `Meeting Minutes` · `会议纪要与待办` · `AI meeting notes` · `meeting transcription` · `meeting summary` · `会议纪要` · `会议转文字` · `office automation` · `AI productivity` · `workflow automation` · `办公自动化` · `效率工具` · `AI办公`

## When to use / 何时使用

Use this skill when the user explicitly requests **meeting minutes**, or when the requested outcome clearly matches this workflow.
...
1. **Load transcript**
2. **Detect explicit decisions**
3. **Detect explicit action-item language**
4. **Preserve source lines for auditability**
5. **Write minutes.json and minutes.md**
6. **Avoid inventing owners or deadlines**
...
```bash
ows doctor meeting-minutes
ows run meeting-minutes -- --help
ows test meeting-minutes
```
...
```bash
npx skills add BryceYuuu/open-workflow-skills --skill meeting-minutes
```

### Install for a specific supported agent

```bash
npx skills add BryceYuuu/open-workflow-skills --skill meeting-minutes --agent codex
```
...
> 使用 `meeting-minutes` 工作流处理我提供的输入。先检查依赖和权限，不覆盖源文件；执行后给我输出路径、验证结果和需要人工审核的部分。

**English**

> Use the `meeting-minutes` workflow for the provided input. Check dependencies and permissions first, preserve the source, validate the result, and report outputs plus any human-review items.
