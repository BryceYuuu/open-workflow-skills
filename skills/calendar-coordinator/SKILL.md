...
description: "Find mutually available meeting windows across calendars and propose or create events after confirmation. Use when the user asks for calendar coordinator or an equivalent workflow."
license: MIT
compatibility: "Cross-agent Agent Skills format. Core stack: Calendar connector. Check runtime dependencies before execution."
metadata:
  ...
  status: "integration"
  automation-level: "L4"
  search-keywords: "calendar-coordinator, Calendar Coordinator, 会议时间协调, AI calendar assistant, meeting scheduler, calendar automation, 日程协调, 自动约会议, office automation, AI productivity, workflow automation, 办公自动化, 效率工具, AI办公"
  quality-score: "4.0/5"
---
# Calendar Coordinator / 会议时间协调

## Purpose / 用途

**中文**

读取参与者日历和时区，寻找共同空档；写入日历前要求确认。

**English**

Find mutually available meeting windows across calendars and propose or create events after confirmation.
...
`calendar-coordinator` · `Calendar Coordinator` · `会议时间协调` · `AI calendar assistant` · `meeting scheduler` · `calendar automation` · `日程协调` · `自动约会议` · `office automation` · `AI productivity` · `workflow automation` · `办公自动化` · `效率工具` · `AI办公`

## When to use / 何时使用

Use this skill when the user explicitly requests **calendar coordinator**, or when the requested outcome clearly matches this workflow.
...
```bash
npx skills add BryceYuuu/open-workflow-skills --skill calendar-coordinator
```

### Install for a specific supported agent

```bash
npx skills add BryceYuuu/open-workflow-skills --skill calendar-coordinator --agent codex
```
...
> 使用 `calendar-coordinator` 工作流处理我提供的输入。先检查依赖和权限，不覆盖源文件；执行后给我输出路径、验证结果和需要人工审核的部分。

**English**

> Use the `calendar-coordinator` workflow for the provided input. Check dependencies and permissions first, preserve the source, validate the result, and report outputs plus any human-review items.
