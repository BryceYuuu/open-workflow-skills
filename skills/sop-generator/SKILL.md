---
name: sop-generator
description: "Convert screen recordings, transcripts, or operation notes into a step-by-step SOP with screenshots and validation checkpoints. Use when the user asks for sop generator or an equivalent workflow."
license: MIT
compatibility: "Cross-agent Agent Skills format. Core stack: Video/frame extraction / ASR / LLM. Check runtime dependencies before execution."
metadata:
  ...
  search-keywords: "sop-generator, SOP Generator, 操作流程转 SOP, AI SOP generator, screen recording to SOP, process documentation, SOP自动生成, 操作手册生成, office automation, AI productivity, workflow automation, 办公自动化, 效率工具, AI办公"
  quality-score: "3.5/5"
---
# SOP Generator / 操作流程转 SOP

## Purpose / 用途

**中文**

把录屏、文字稿或操作记录整理成带截图和验证点的 SOP；仅凭录屏可能漏掉不可见操作。

**English**

Convert screen recordings, transcripts, or operation notes into a step-by-step SOP with screenshots and validation checkpoints.
...
`sop-generator` · `SOP Generator` · `操作流程转 SOP` · `AI SOP generator` · `screen recording to SOP` · `process documentation` · `SOP自动生成` · `操作手册生成` · `office automation` · `AI productivity` · `workflow automation` · `办公自动化` · `效率工具` · `AI办公`

## When to use / 何时使用

Use this skill when the user explicitly requests **sop generator**, or when the requested outcome clearly matches this workflow.
...
`Video/frame extraction / ASR / LLM`
...
```bash
npx skills add BryceYuuu/open-workflow-skills --skill sop-generator
```

### Install for a specific supported agent

```bash
npx skills add BryceYuuu/open-workflow-skills --skill sop-generator --agent codex
```
...
> 使用 `sop-generator` 工作流处理我提供的输入。先检查依赖和权限，不覆盖源文件；执行后给我输出路径、验证结果和需要人工审核的部分。

**English**

> Use the `sop-generator` workflow for the provided input. Check dependencies and permissions first, preserve the source, validate the result, and report outputs plus any human-review items.
