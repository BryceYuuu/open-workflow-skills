...
description: "Extract public or authorized website data into a declared schema using browser automation while respecting access controls and site policies. Use when the user asks for web data extractor or an equivalent workflow."
license: MIT
compatibility: "Cross-agent Agent Skills format. Core stack: Playwright / BeautifulSoup / Pandas. Check runtime dependencies before execution."
metadata:
  ...
  search-keywords: "web-data-extractor, Web Data Extractor, 网页结构化数据采集, AI web scraper, web scraping, website data extraction, crawler, 网页爬虫, 网页数据采集, data automation, AI data analysis, Excel automation, analytics workflow, 数据自动化"
  quality-score: "4.0/5"
---
# Web Data Extractor / 网页结构化数据采集

## Purpose / 用途

**中文**

通过浏览器自动化把公开或已授权网页提取成指定 Schema；需遵守登录权限、反爬限制和站点规则。

**English**

Extract public or authorized website data into a declared schema using browser automation while respecting access controls and site policies.
...
`web-data-extractor` · `Web Data Extractor` · `网页结构化数据采集` · `AI web scraper` · `web scraping` · `website data extraction` · `crawler` · `网页爬虫` · `网页数据采集` · `data automation` · `AI data analysis` · `Excel automation` · `analytics workflow` · `数据自动化`

## When to use / 何时使用

Use this skill when the user explicitly requests **web data extractor**, or when the requested outcome clearly matches this workflow.
...
- Python: `python3 -m pip install playwright beautifulsoup4 pandas`
- Browser: `python3 -m playwright install chromium`
...
```bash
npx skills add BryceYuuu/open-workflow-skills --skill web-data-extractor
```

### Install for a specific supported agent

```bash
npx skills add BryceYuuu/open-workflow-skills --skill web-data-extractor --agent codex
```
...
> 使用 `web-data-extractor` 工作流处理我提供的输入。先检查依赖和权限，不覆盖源文件；执行后给我输出路径、验证结果和需要人工审核的部分。

**English**

> Use the `web-data-extractor` workflow for the provided input. Check dependencies and permissions first, preserve the source, validate the result, and report outputs plus any human-review items.
