...
description: "Analyze message context and generate multiple reply options with distinct tone and intent while preserving known facts. Use when the user asks for reply assistant or an equivalent workflow."
license: MIT
...
metadata:
  ...
  search-keywords: "reply-assistant, Reply Assistant, 智能回复助手, AI reply assistant, message reply generator, DM reply, 自动回复, 消息回复, communication automation, social automation, AI writing assistant, 沟通自动化, 社交媒体自动化, AI写作"
  quality-score: "4.5/5"
---
# Reply Assistant / 智能回复助手

## Purpose / 用途

**中文**

分析消息上下文并生成不同语气与目的的回复选项，同时保持事实一致。

**English**

Analyze message context and generate multiple reply options with distinct tone and intent while preserving known facts.
...
`reply-assistant` · `Reply Assistant` · `智能回复助手` · `AI reply assistant` · `message reply generator` · `DM reply` · `自动回复` · `消息回复` · `communication automation` · `social automation` · `AI writing assistant` · `沟通自动化` · `社交媒体自动化` · `AI写作`

## When to use / 何时使用

Use this skill when the user explicitly requests **reply assistant**, or when the requested outcome clearly matches this workflow.
...
```bash
npx skills add BryceYuuu/open-workflow-skills --skill reply-assistant
```

### Install for a specific supported agent

```bash
npx skills add BryceYuuu/open-workflow-skills --skill reply-assistant --agent codex
```
...
> 使用 `reply-assistant` 工作流处理我提供的输入。先检查依赖和权限，不覆盖源文件；执行后给我输出路径、验证结果和需要人工审核的部分。

**English**

> Use the `reply-assistant` workflow for the provided input. Check dependencies and permissions first, preserve the source, validate the result, and report outputs plus any human-review items.
