...
description: "Generate a consistent SVG icon set and normalize paths, viewBox, stroke, naming, and output validity. Use when the user asks for svg icon generator or an equivalent workflow."
license: MIT
...
metadata:
  ...
  quality-score: "3.5/5"
---
# SVG Icon Generator / SVG 图标生成

## Purpose / 用途

**中文**

生成统一风格 SVG 图标集，并规范路径、ViewBox、Stroke、命名和文件合法性。

**English**

Generate a consistent SVG icon set and normalize paths, viewBox, stroke, naming, and output validity.
...
`svg-icon-generator` · `SVG Icon Generator` · `SVG 图标生成` · `AI SVG generator` · `icon set generator` · `SVG icons` · `SVG图标` · `图标生成` · `AI design` · `design automation` · `creative automation` · `AI设计` · `设计自动化` · `创意工作流`

## When to use / 何时使用

Use this skill when the user explicitly requests **svg icon generator**, or when the requested outcome clearly matches this workflow.
...
`SVG tools / optional image model`
...
```bash
npx skills add BryceYuuu/open-workflow-skills --skill svg-icon-generator
```

### Install for a specific supported agent

```bash
npx skills add BryceYuuu/open-workflow-skills --skill svg-icon-generator --agent codex
```
...
> 使用 `svg-icon-generator` 工作流处理我提供的输入。先检查依赖和权限，不覆盖源文件；执行后给我输出路径、验证结果和需要人工审核的部分。

**English**

> Use the `svg-icon-generator` workflow for the provided input. Check dependencies and permissions first, preserve the source, validate the result, and report outputs plus any human-review items.
