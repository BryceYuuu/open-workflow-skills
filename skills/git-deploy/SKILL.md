---
name: git-deploy
description: "Orchestrate build, test, artifact creation, approval, and deployment after repository changes; production deployment is approval-gated. Use when the user asks for git deploy or an equivalent workflow."
license: MIT
compatibility: "Cross-agent Agent Skills format. Core stack: Git / CI/CD / deployment tooling. Check runtime dependencies before execution."
metadata:
  ...
  search-keywords: "git-deploy, Git Deploy, Git 自动部署, Git deployment, CI CD automation, GitHub Actions deploy, 自动部署, CI/CD, DevOps automation, system automation, infrastructure automation, DevOps自动化, 运维自动化, 系统自动化"
  quality-score: "4.0/5"
---
# Git Deploy / Git 自动部署
...
在代码变更后编排 Build、Test、Artifact、Approval 和 Deploy；生产部署默认需要批准。

**English**

Orchestrate build, test, artifact creation, approval, and deployment after repository changes; production deployment is approval-gated.
...
`git-deploy` · `Git Deploy` · `Git 自动部署` · `Git deployment` · `CI CD automation` · `GitHub Actions deploy` · `自动部署` · `CI/CD` · `DevOps automation` · `system automation` · `infrastructure automation` · `DevOps自动化` · `运维自动化` · `系统自动化`

## When to use / 何时使用

Use this skill when the user explicitly requests **git deploy**, or when the requested outcome clearly matches this workflow.
...
```bash
npx skills add BryceYuuu/open-workflow-skills --skill git-deploy
```

### Install for a specific supported agent

```bash
npx skills add BryceYuuu/open-workflow-skills --skill git-deploy --agent codex
```
...
> 使用 `git-deploy` 工作流处理我提供的输入。先检查依赖和权限，不覆盖源文件；执行后给我输出路径、验证结果和需要人工审核的部分。

**English**

> Use the `git-deploy` workflow for the provided input. Check dependencies and permissions first, preserve the source, validate the result, and report outputs plus any human-review items.
