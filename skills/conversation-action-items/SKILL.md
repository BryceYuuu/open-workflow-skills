---
name: conversation-action-items
description: "Extract explicit commitments, tasks, owners, deadlines, and context from chat or meeting transcripts. Use when the user asks for conversation action items or an equivalent workflow."
license: MIT
...
metadata:
  ...
  status: "production"
  automation-level: "L2"
  search-keywords: "conversation-action-items, Conversation Action Items, 聊天待办提取, action item extractor, conversation to tasks, meeting action items, 待办提取, 聊天任务提取, communication automation, social automation, AI writing assistant, 沟通自动化, 社交媒体自动化, AI写作"
  quality-score: "4.5/5"
---
# Conversation Action Items / 聊天待办提取

## Purpose / 用途

**中文**

从聊天或会议中提取明确承诺、任务、负责人、截止日期和上下文，避免凭空猜测。

**English**

Extract explicit commitments, tasks, owners, deadlines, and context from chat or meeting transcripts.
...
`conversation-action-items` · `Conversation Action Items` · `聊天待办提取` · `action item extractor` · `conversation to tasks` · `meeting action items` · `待办提取` · `聊天任务提取` · `communication automation` · `social automation` · `AI writing assistant` · `沟通自动化` · `社交媒体自动化` · `AI写作`

## When to use / 何时使用

Use this skill when the user explicitly requests **conversation action items**, or when the requested outcome clearly matches this workflow.
...
```bash
npx skills add BryceYuuu/open-workflow-skills --skill conversation-action-items
```

### Install for a specific supported agent

```bash
npx skills add BryceYuuu/open-workflow-skills --skill conversation-action-items --agent codex
```
...
> 使用 `conversation-action-items` 工作流处理我提供的输入。先检查依赖和权限，不覆盖源文件；执行后给我输出路径、验证结果和需要人工审核的部分。

**English**

> Use the `conversation-action-items` workflow for the provided input. Check dependencies and permissions first, preserve the source, validate the result, and report outputs plus any human-review items.
