...
description: "Plan and execute incremental framework, language, SDK, or architecture migrations with dependency audits and validation. Use when the user asks for framework migration or an equivalent workflow."
license: MIT
compatibility: "Cross-agent Agent Skills format. Core stack: Git / build tools / package managers. Check runtime dependencies before execution."
metadata:
  ...
  search-keywords: "framework-migration, Framework Migration, 技术栈迁移, code migration, framework migration, JavaScript to TypeScript, 框架迁移"
  quality-score: "3.5/5"
---
# Framework Migration / 技术栈迁移

## Purpose / 用途

**中文**

规划并分阶段执行框架、语言、SDK 或架构迁移，包含依赖审计和验证。

**English**

Plan and execute incremental framework, language, SDK, or architecture migrations with dependency audits and validation.
...
`framework-migration` · `Framework Migration` · `技术栈迁移` · `code migration` · `framework migration` · `JavaScript to TypeScript` · `框架迁移`

## When to use / 何时使用

Use this skill when the user explicitly requests **framework migration**, or when the requested outcome clearly matches this workflow.
...
```bash
npx skills add BryceYuuu/open-workflow-skills --skill framework-migration
```

### Install for a specific supported agent

```bash
npx skills add BryceYuuu/open-workflow-skills --skill framework-migration --agent codex
```
...
> 使用 `framework-migration` 工作流处理我提供的输入。先检查依赖和权限，不覆盖源文件；执行后给我输出路径、验证结果和需要人工审核的部分。

**English**

> Use the `framework-migration` workflow for the provided input. Check dependencies and permissions first, preserve the source, validate the result, and report outputs plus any human-review items.
