...
description: "Extract dominant colors and map them into semantic design tokens such as primary, surface, text, border, and accent. Use when the user asks for palette extractor or an equivalent workflow."
license: MIT
...
metadata:
  ...
  quality-score: "5.0/5"
---
# Palette Extractor / 配色提取

## Purpose / 用途

**中文**

从参考图提取主要颜色并映射成 Primary、Surface、Text、Border、Accent 等语义色。

**English**

Extract dominant colors and map them into semantic design tokens such as primary, surface, text, border, and accent.
...
`palette-extractor` · `Palette Extractor` · `配色提取` · `color palette extractor` · `image color palette` · `design tokens` · `色板生成` · `AI design` · `design automation` · `creative automation` · `AI设计` · `设计自动化` · `创意工作流`

## When to use / 何时使用

Use this skill when the user explicitly requests **palette extractor**, or when the requested outcome clearly matches this workflow.
...
```bash
npx skills add BryceYuuu/open-workflow-skills --skill palette-extractor
```

### Install for a specific supported agent

```bash
npx skills add BryceYuuu/open-workflow-skills --skill palette-extractor --agent codex
```
...
> 使用 `palette-extractor` 工作流处理我提供的输入。先检查依赖和权限，不覆盖源文件；执行后给我输出路径、验证结果和需要人工审核的部分。

**English**

> Use the `palette-extractor` workflow for the provided input. Check dependencies and permissions first, preserve the source, validate the result, and report outputs plus any human-review items.
