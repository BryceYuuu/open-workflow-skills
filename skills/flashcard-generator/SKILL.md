...
description: "Convert notes or source material into concise spaced-repetition cards and export compatible CSV or structured formats. Use when the user asks for flashcard generator or an equivalent workflow."
license: MIT
compatibility: "Cross-agent Agent Skills format. Core stack: LLM / optional Anki tooling. Check runtime dependencies before execution."
metadata:
  ...
  search-keywords: "flashcard-generator, Flashcard Generator, 学习卡片生成, AI flashcards, Anki generator, study cards, Anki卡片, 记忆卡片, AI research, learning automation, education AI, 研究助手, 学习自动化, AI教育"
  quality-score: "4.5/5"
---
# Flashcard Generator / 学习卡片生成

## Purpose / 用途

**中文**

把笔记或学习材料转换成适合间隔重复的卡片，并输出 Anki 兼容 CSV 或结构化格式。

**English**

Convert notes or source material into concise spaced-repetition cards and export compatible CSV or structured formats.
...
`flashcard-generator` · `Flashcard Generator` · `学习卡片生成` · `AI flashcards` · `Anki generator` · `study cards` · `Anki卡片` · `记忆卡片` · `AI research` · `learning automation` · `education AI` · `研究助手` · `学习自动化` · `AI教育`

## When to use / 何时使用

Use this skill when the user explicitly requests **flashcard generator**, or when the requested outcome clearly matches this workflow.
...
```bash
npx skills add BryceYuuu/open-workflow-skills --skill flashcard-generator
```

### Install for a specific supported agent

```bash
npx skills add BryceYuuu/open-workflow-skills --skill flashcard-generator --agent codex
```
...
> 使用 `flashcard-generator` 工作流处理我提供的输入。先检查依赖和权限，不覆盖源文件；执行后给我输出路径、验证结果和需要人工审核的部分。

**English**

> Use the `flashcard-generator` workflow for the provided input. Check dependencies and permissions first, preserve the source, validate the result, and report outputs plus any human-review items.
