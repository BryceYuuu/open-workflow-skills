...
description: "Monitor authorized public competitor sources, detect meaningful changes, and generate dated intelligence digests. Use when the user asks for competitor monitor or an equivalent workflow."
license: MIT
compatibility: "Cross-agent Agent Skills format. Core stack: Browser / scheduler / diff / notifications. Check runtime dependencies before execution."
metadata:
  ...
  status: "integration"
  automation-level: "L4"
  search-keywords: "competitor-monitor, Competitor Monitor, 竞争对手自动监控, competitor monitoring, competitive intelligence, competitor tracking, 竞品监控, 竞争对手分析, business automation, growth automation, AI business, 商业自动化, 增长自动化, AI商业"
  quality-score: "4.0/5"
---
# Competitor Monitor / 竞争对手自动监控

## Purpose / 用途

**中文**

监控允许访问的竞争对手公开来源，检测真正变化并生成带日期的情报摘要。

**English**

Monitor authorized public competitor sources, detect meaningful changes, and generate dated intelligence digests.
...
`competitor-monitor` · `Competitor Monitor` · `竞争对手自动监控` · `competitor monitoring` · `competitive intelligence` · `competitor tracking` · `竞品监控` · `竞争对手分析` · `business automation` · `growth automation` · `AI business` · `商业自动化` · `增长自动化` · `AI商业`

## When to use / 何时使用

Use this skill when the user explicitly requests **competitor monitor**, or when the requested outcome clearly matches this workflow.
...
`Browser / scheduler / diff / notifications`
...
```bash
npx skills add BryceYuuu/open-workflow-skills --skill competitor-monitor
```

### Install for a specific supported agent

```bash
npx skills add BryceYuuu/open-workflow-skills --skill competitor-monitor --agent codex
```
...
> 使用 `competitor-monitor` 工作流处理我提供的输入。先检查依赖和权限，不覆盖源文件；执行后给我输出路径、验证结果和需要人工审核的部分。

**English**

> Use the `competitor-monitor` workflow for the provided input. Check dependencies and permissions first, preserve the source, validate the result, and report outputs plus any human-review items.
