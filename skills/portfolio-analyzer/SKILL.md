...
description: "Calculate portfolio return, volatility, concentration, allocation, exposure, and correlation from provided holdings and market data. Use when the user asks for portfolio analyzer or an equivalent workflow."
license: MIT
compatibility: "Cross-agent Agent Skills format. Core stack: Python / market data. Check runtime dependencies before execution."
metadata:
  ...
  search-keywords: "portfolio-analyzer, Portfolio Analyzer, 投资组合分析, portfolio analysis, investment risk analysis, asset allocation, 持仓分析, business automation, growth automation, AI business, 商业自动化, 增长自动化, AI商业"
  quality-score: "4.5/5"
---
...
根据持仓和市场数据计算收益、波动、集中度、配置、风险暴露和相关性。

**English**

Calculate portfolio return, volatility, concentration, allocation, exposure, and correlation from provided holdings and market data.
...
`portfolio-analyzer` · `Portfolio Analyzer` · `投资组合分析` · `portfolio analysis` · `investment risk analysis` · `asset allocation` · `持仓分析` · `business automation` · `growth automation` · `AI business` · `商业自动化` · `增长自动化` · `AI商业`

## When to use / 何时使用

Use this skill when the user explicitly requests **portfolio analyzer**, or when the requested outcome clearly matches this workflow.
...
```bash
npx skills add BryceYuuu/open-workflow-skills --skill portfolio-analyzer
```

### Install for a specific supported agent

```bash
npx skills add BryceYuuu/open-workflow-skills --skill portfolio-analyzer --agent codex
```
...
> 使用 `portfolio-analyzer` 工作流处理我提供的输入。先检查依赖和权限，不覆盖源文件；执行后给我输出路径、验证结果和需要人工审核的部分。

**English**

> Use the `portfolio-analyzer` workflow for the provided input. Check dependencies and permissions first, preserve the source, validate the result, and report outputs plus any human-review items.
