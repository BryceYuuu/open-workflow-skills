---
name: code-refactor
description: "Analyze a repository, create an incremental refactor plan, modify code in small steps, and validate each step with tests. Use when the user asks for code refactor or an equivalent workflow."
license: MIT
compatibility: "Cross-agent Agent Skills format. Core stack: Git / project test tools. Check runtime dependencies before execution."
metadata:
  ...
  automation-level: "L2"
  search-keywords: "code-refactor, Code Refactor, 代码重构, AI refactoring, code cleanup, refactor agent, 代码优化"
  quality-score: "4.5/5"
---
# Code Refactor / 代码重构
...
分析代码仓库，先制定增量重构计划，再小步修改并通过测试验证。

**English**

Analyze a repository, create an incremental refactor plan, modify code in small steps, and validate each step with tests.
...
`code-refactor` · `Code Refactor` · `代码重构` · `AI refactoring` · `code cleanup` · `refactor agent` · `代码优化`

## When to use / 何时使用

Use this skill when the user explicitly requests **code refactor**, or when the requested outcome clearly matches this workflow.
...
```bash
npx skills add BryceYuuu/open-workflow-skills --skill code-refactor
```

### Install for a specific supported agent

```bash
npx skills add BryceYuuu/open-workflow-skills --skill code-refactor --agent codex
```
...
> 使用 `code-refactor` 工作流处理我提供的输入。先检查依赖和权限，不覆盖源文件；执行后给我输出路径、验证结果和需要人工审核的部分。

**English**

> Use the `code-refactor` workflow for the provided input. Check dependencies and permissions first, preserve the source, validate the result, and report outputs plus any human-review items.
