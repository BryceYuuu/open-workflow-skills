---
name: study-plan
description: "Create adaptive study schedules from target, deadline, current level, available time, and spaced-review requirements. Use when the user asks for study plan or an equivalent workflow."
license: MIT
compatibility: "Cross-agent Agent Skills format. Core stack: LLM / optional calendar. Check runtime dependencies before execution."
metadata:
  ...
  search-keywords: "study-plan, Study Plan, AI 学习计划, AI study planner, exam study plan, learning schedule, 学习计划, 复习计划, AI research, learning automation, education AI, 研究助手, 学习自动化, AI教育"
  quality-score: "4.5/5"
---
# Study Plan / AI 学习计划
...
根据目标、截止日期、当前水平、可用时间和复习周期生成可执行学习计划。

**English**

Create adaptive study schedules from target, deadline, current level, available time, and spaced-review requirements.
...
`study-plan` · `Study Plan` · `AI 学习计划` · `AI study planner` · `exam study plan` · `learning schedule` · `学习计划` · `复习计划` · `AI research` · `learning automation` · `education AI` · `研究助手` · `学习自动化` · `AI教育`

## When to use / 何时使用

Use this skill when the user explicitly requests **study plan**, or when the requested outcome clearly matches this workflow.
...
```bash
npx skills add BryceYuuu/open-workflow-skills --skill study-plan
```

### Install for a specific supported agent

```bash
npx skills add BryceYuuu/open-workflow-skills --skill study-plan --agent codex
```
...
> 使用 `study-plan` 工作流处理我提供的输入。先检查依赖和权限，不覆盖源文件；执行后给我输出路径、验证结果和需要人工审核的部分。

**English**

> Use the `study-plan` workflow for the provided input. Check dependencies and permissions first, preserve the source, validate the result, and report outputs plus any human-review items.
