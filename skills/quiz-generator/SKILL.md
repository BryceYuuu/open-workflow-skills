---
name: quiz-generator
description: "Generate assessments aligned to source material with varied difficulty, plausible distractors, verified answers, and explanations. Use when the user asks for quiz generator or an equivalent workflow."
license: MIT
compatibility: "Cross-agent Agent Skills format. Core stack: LLM / optional validator. Check runtime dependencies before execution."
metadata:
  ...
  search-keywords: "quiz-generator, Quiz Generator, AI 出题与解析, AI quiz generator, test question generator, practice test, AI出题, 测试题生成, AI research, learning automation, education AI, 研究助手, 学习自动化, AI教育"
  quality-score: "4.5/5"
---
# Quiz Generator / AI 出题与解析

## Purpose / 用途

**中文**

根据学习材料生成不同难度的测试题、合理干扰项、经过校验的答案和解析。

**English**

Generate assessments aligned to source material with varied difficulty, plausible distractors, verified answers, and explanations.
...
`quiz-generator` · `Quiz Generator` · `AI 出题与解析` · `AI quiz generator` · `test question generator` · `practice test` · `AI出题` · `测试题生成` · `AI research` · `learning automation` · `education AI` · `研究助手` · `学习自动化` · `AI教育`

## When to use / 何时使用

Use this skill when the user explicitly requests **quiz generator**, or when the requested outcome clearly matches this workflow.
...
```bash
npx skills add BryceYuuu/open-workflow-skills --skill quiz-generator
```

### Install for a specific supported agent

```bash
npx skills add BryceYuuu/open-workflow-skills --skill quiz-generator --agent codex
```
...
> 使用 `quiz-generator` 工作流处理我提供的输入。先检查依赖和权限，不覆盖源文件；执行后给我输出路径、验证结果和需要人工审核的部分。

**English**

> Use the `quiz-generator` workflow for the provided input. Check dependencies and permissions first, preserve the source, validate the result, and report outputs plus any human-review items.
