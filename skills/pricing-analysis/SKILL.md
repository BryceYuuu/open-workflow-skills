...
description: "Model pricing scenarios from costs, margin targets, customer segments, and competitor inputs without claiming a single objectively optimal price. Use when the user asks for pricing analysis or an equivalent workflow."
license: MIT
compatibility: "Cross-agent Agent Skills format. Core stack: Spreadsheet/Python / optional market data. Check runtime dependencies before execution."
metadata:
  ...
  search-keywords: "pricing-analysis, Pricing Analysis, 定价分析, AI pricing, pricing strategy, price optimization, 价格策略, business automation, growth automation, AI business, 商业自动化, 增长自动化, AI商业"
  quality-score: "3.5/5"
---
...
根据成本、毛利目标、用户分层和竞品输入模拟价格方案，不声称存在唯一“最佳价格”。

**English**

Model pricing scenarios from costs, margin targets, customer segments, and competitor inputs without claiming a single objectively optimal price.
...
`pricing-analysis` · `Pricing Analysis` · `定价分析` · `AI pricing` · `pricing strategy` · `price optimization` · `价格策略` · `business automation` · `growth automation` · `AI business` · `商业自动化` · `增长自动化` · `AI商业`

## When to use / 何时使用

Use this skill when the user explicitly requests **pricing analysis**, or when the requested outcome clearly matches this workflow.
...
`Spreadsheet/Python / optional market data`
...
```bash
npx skills add BryceYuuu/open-workflow-skills --skill pricing-analysis
```

### Install for a specific supported agent

```bash
npx skills add BryceYuuu/open-workflow-skills --skill pricing-analysis --agent codex
```
...
> 使用 `pricing-analysis` 工作流处理我提供的输入。先检查依赖和权限，不覆盖源文件；执行后给我输出路径、验证结果和需要人工审核的部分。

**English**

> Use the `pricing-analysis` workflow for the provided input. Check dependencies and permissions first, preserve the source, validate the result, and report outputs plus any human-review items.
