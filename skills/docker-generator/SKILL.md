...
description: "Analyze an application, generate Dockerfile and Compose configuration, build images, and validate runtime behavior. Use when the user asks for docker generator or an equivalent workflow."
license: MIT
compatibility: "Cross-agent Agent Skills format. Core stack: Docker / Compose. Check runtime dependencies before execution."
metadata:
  ...
  automation-level: "L2"
  search-keywords: "docker-generator, Docker Generator, Docker 配置生成器, AI Dockerfile, Docker Compose generator, containerize app, Docker生成, 容器化, DevOps automation, system automation, infrastructure automation, DevOps自动化, 运维自动化, 系统自动化"
  quality-score: "4.5/5"
---
# Docker Generator / Docker 配置生成器

## Purpose / 用途

**中文**

分析项目运行环境，生成 Dockerfile/Compose，并实际 Build 和运行验证。

**English**

Analyze an application, generate Dockerfile and Compose configuration, build images, and validate runtime behavior.
...
`docker-generator` · `Docker Generator` · `Docker 配置生成器` · `AI Dockerfile` · `Docker Compose generator` · `containerize app` · `Docker生成` · `容器化` · `DevOps automation` · `system automation` · `infrastructure automation` · `DevOps自动化` · `运维自动化` · `系统自动化`

## When to use / 何时使用

Use this skill when the user explicitly requests **docker generator**, or when the requested outcome clearly matches this workflow.
...
- Install Docker Desktop or Docker Engine + Compose for your OS.
...
```bash
npx skills add BryceYuuu/open-workflow-skills --skill docker-generator
```

### Install for a specific supported agent

```bash
npx skills add BryceYuuu/open-workflow-skills --skill docker-generator --agent codex
```
...
> 使用 `docker-generator` 工作流处理我提供的输入。先检查依赖和权限，不覆盖源文件；执行后给我输出路径、验证结果和需要人工审核的部分。

**English**

> Use the `docker-generator` workflow for the provided input. Check dependencies and permissions first, preserve the source, validate the result, and report outputs plus any human-review items.
