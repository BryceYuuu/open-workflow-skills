---
name: backup-manager
description: "Design and run full or incremental backups with encryption, retention, integrity checks, and restore verification. Use when the user asks for backup manager or an equivalent workflow."
license: MIT
...
设计并执行完整或增量备份，包含加密、保留策略、完整性检查和恢复验证。

**English**

Design and run full or incremental backups with encryption, retention, integrity checks, and restore verification.
...
`backup-manager` · `Backup Manager` · `自动备份管理` · `automatic backup` · `incremental backup` · `backup automation` · `自动备份` · `增量备份` · `DevOps automation` · `system automation` · `infrastructure automation` · `DevOps自动化` · `运维自动化` · `系统自动化`

## When to use / 何时使用

Use this skill when the user explicitly requests **backup manager**, or when the requested outcome clearly matches this workflow.
...
`rsync/restic/borg or cloud backup`
...
```bash
npx skills add BryceYuuu/open-workflow-skills --skill backup-manager
```

### Install for a specific supported agent

```bash
npx skills add BryceYuuu/open-workflow-skills --skill backup-manager --agent codex
```
...
> 使用 `backup-manager` 工作流处理我提供的输入。先检查依赖和权限，不覆盖源文件；执行后给我输出路径、验证结果和需要人工审核的部分。

**English**

> Use the `backup-manager` workflow for the provided input. Check dependencies and permissions first, preserve the source, validate the result, and report outputs plus any human-review items.
