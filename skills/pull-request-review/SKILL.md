...
description: "Review diffs for correctness, security, performance, maintainability, tests, and breaking changes while prioritizing actionable findings. Use when the user asks for pull request review or an equivalent workflow."
license: MIT
...
metadata:
  ...
  quality-score: "4.5/5"
---
# Pull Request Review / PR 代码审查

## Purpose / 用途

**中文**

审查 PR 的正确性、安全、性能、可维护性、测试和破坏性变化，并优先报告真正可执行的问题。

**English**

Review diffs for correctness, security, performance, maintainability, tests, and breaking changes while prioritizing actionable findings.
...
`pull-request-review` · `Pull Request Review` · `PR 代码审查` · `AI code review` · `PR review` · `GitHub code review` · `代码审查` · `PR审查`

## When to use / 何时使用

Use this skill when the user explicitly requests **pull request review**, or when the requested outcome clearly matches this workflow.
...
`Git/GitHub optional / static analysis`
...
```bash
npx skills add BryceYuuu/open-workflow-skills --skill pull-request-review
```

### Install for a specific supported agent

```bash
npx skills add BryceYuuu/open-workflow-skills --skill pull-request-review --agent codex
```
...
> 使用 `pull-request-review` 工作流处理我提供的输入。先检查依赖和权限，不覆盖源文件；执行后给我输出路径、验证结果和需要人工审核的部分。

**English**

> Use the `pull-request-review` workflow for the provided input. Check dependencies and permissions first, preserve the source, validate the result, and report outputs plus any human-review items.
