---
name: inbox-triage
description: "Classify unread email by urgency and actionability, summarize threads, and create reply drafts without sending by default. Use when the user asks for inbox triage or an equivalent workflow."
license: MIT
compatibility: "Cross-agent Agent Skills format. Core stack: Email connector / LLM. Check runtime dependencies before execution."
metadata:
  ...
  status: "integration"
  automation-level: "L4"
  search-keywords: "inbox-triage, Inbox Triage, AI 邮件分流, AI email assistant, email triage, Gmail automation, email summarizer, 邮件自动分类, 邮箱助手, office automation, AI productivity, workflow automation, 办公自动化, 效率工具"
  quality-score: "4.5/5"
---
# Inbox Triage / AI 邮件分流

## Purpose / 用途

**中文**

对未读邮件进行优先级分类、线程摘要和回复草稿生成，默认不自动发送。

**English**

Classify unread email by urgency and actionability, summarize threads, and create reply drafts without sending by default.
...
`inbox-triage` · `Inbox Triage` · `AI 邮件分流` · `AI email assistant` · `email triage` · `Gmail automation` · `email summarizer` · `邮件自动分类` · `邮箱助手` · `office automation` · `AI productivity` · `workflow automation` · `办公自动化` · `效率工具`

## When to use / 何时使用

Use this skill when the user explicitly requests **inbox triage**, or when the requested outcome clearly matches this workflow.
...
```bash
npx skills add BryceYuuu/open-workflow-skills --skill inbox-triage
```

### Install for a specific supported agent

```bash
npx skills add BryceYuuu/open-workflow-skills --skill inbox-triage --agent codex
```
...
> 使用 `inbox-triage` 工作流处理我提供的输入。先检查依赖和权限，不覆盖源文件；执行后给我输出路径、验证结果和需要人工审核的部分。

**English**

> Use the `inbox-triage` workflow for the provided input. Check dependencies and permissions first, preserve the source, validate the result, and report outputs plus any human-review items.
