...
description: "Turn a product brief into information architecture, copy, responsive UI, frontend code, QA, and optionally deployment through connected infrastructure. Use when the user asks for landing page builder or an equivalent workflow."
license: MIT
compatibility: "Cross-agent Agent Skills format. Core stack: Frontend stack / browser / deployment optional. Check runtime dependencies before execution."
metadata:
  ...
  search-keywords: "landing-page-builder, Landing Page Builder, 落地页生成器, AI landing page, website generator, landing page code, 落地页生成, AI建站, AI design, design automation, creative automation, AI设计, 设计自动化, 创意工作流"
  quality-score: "4.0/5"
---
# Landing Page Builder / 落地页生成器

## Purpose / 用途

**中文**

把产品说明转换成信息架构、文案、响应式 UI、前端代码和 QA，并可通过已连接基础设施部署。

**English**

Turn a product brief into information architecture, copy, responsive UI, frontend code, QA, and optionally deployment through connected infrastructure.
...
`landing-page-builder` · `Landing Page Builder` · `落地页生成器` · `AI landing page` · `website generator` · `landing page code` · `落地页生成` · `AI建站` · `AI design` · `design automation` · `creative automation` · `AI设计` · `设计自动化` · `创意工作流`

## When to use / 何时使用

Use this skill when the user explicitly requests **landing page builder**, or when the requested outcome clearly matches this workflow.
...
`Frontend stack / browser / deployment optional`
...
```bash
npx skills add BryceYuuu/open-workflow-skills --skill landing-page-builder
```

### Install for a specific supported agent

```bash
npx skills add BryceYuuu/open-workflow-skills --skill landing-page-builder --agent codex
```
...
> 使用 `landing-page-builder` 工作流处理我提供的输入。先检查依赖和权限，不覆盖源文件；执行后给我输出路径、验证结果和需要人工审核的部分。

**English**

> Use the `landing-page-builder` workflow for the provided input. Check dependencies and permissions first, preserve the source, validate the result, and report outputs plus any human-review items.
