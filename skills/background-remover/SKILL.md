...
description: "Segment image subjects, refine masks, export transparent PNGs, and optionally composite approved replacement backgrounds. Use when the user asks for background remover or an equivalent workflow."
license: MIT
compatibility: "Cross-agent Agent Skills format. Core stack: rembg/segmentation model / image tools. Check runtime dependencies before execution."
metadata:
  ...
  search-keywords: "background-remover, Background Remover, 自动抠图, AI background remover, remove image background, transparent PNG, 去背景, AI design, design automation, creative automation, AI设计, 设计自动化, 创意工作流"
  quality-score: "4.5/5"
---
# Background Remover / 自动抠图

## Purpose / 用途

**中文**

识别主体、优化边缘、输出透明 PNG，并可组合用户批准的新背景。

**English**

Segment image subjects, refine masks, export transparent PNGs, and optionally composite approved replacement backgrounds.
...
`background-remover` · `Background Remover` · `自动抠图` · `AI background remover` · `remove image background` · `transparent PNG` · `去背景` · `AI design` · `design automation` · `creative automation` · `AI设计` · `设计自动化` · `创意工作流`

## When to use / 何时使用

Use this skill when the user explicitly requests **background remover**, or when the requested outcome clearly matches this workflow.
...
`rembg/segmentation model / image tools`
...
```bash
npx skills add BryceYuuu/open-workflow-skills --skill background-remover
```

### Install for a specific supported agent

```bash
npx skills add BryceYuuu/open-workflow-skills --skill background-remover --agent codex
```
...
> 使用 `background-remover` 工作流处理我提供的输入。先检查依赖和权限，不覆盖源文件；执行后给我输出路径、验证结果和需要人工审核的部分。

**English**

> Use the `background-remover` workflow for the provided input. Check dependencies and permissions first, preserve the source, validate the result, and report outputs plus any human-review items.
