---
name: api-generator
description: "Turn requirements into API schemas, routes, validation, authentication scaffolding, OpenAPI documentation, and tests. Use when the user asks for api generator or an equivalent workflow."
license: MIT
compatibility: "Cross-agent Agent Skills format. Core stack: Backend framework / OpenAPI / tests. Check runtime dependencies before execution."
metadata:
  ...
  search-keywords: "api-generator, API Generator, 自然语言生成 API, AI API generator, REST API generator, OpenAPI generator, API生成, 接口生成"
  quality-score: "4.0/5"
---
# API Generator / 自然语言生成 API

## Purpose / 用途

**中文**

把需求转换成数据模型、路由、校验、认证脚手架、OpenAPI 文档和测试；生产安全配置需额外审查。

**English**

Turn requirements into API schemas, routes, validation, authentication scaffolding, OpenAPI documentation, and tests.
...
`api-generator` · `API Generator` · `自然语言生成 API` · `AI API generator` · `REST API generator` · `OpenAPI generator` · `API生成` · `接口生成`

## When to use / 何时使用

Use this skill when the user explicitly requests **api generator**, or when the requested outcome clearly matches this workflow.
...
`Backend framework / OpenAPI / tests`
...
```bash
npx skills add BryceYuuu/open-workflow-skills --skill api-generator
```

### Install for a specific supported agent

```bash
npx skills add BryceYuuu/open-workflow-skills --skill api-generator --agent codex
```
...
> 使用 `api-generator` 工作流处理我提供的输入。先检查依赖和权限，不覆盖源文件；执行后给我输出路径、验证结果和需要人工审核的部分。

**English**

> Use the `api-generator` workflow for the provided input. Check dependencies and permissions first, preserve the source, validate the result, and report outputs plus any human-review items.
