...
description: "Analyze long-form video and produce short clip candidates based on topic boundaries, semantic density, and pacing. Use when the user asks for long video to clip candidates or an equivalent workflow."
license: MIT
compatibility: "Cross-agent Agent Skills format. Core stack: FFmpeg / ASR / LLM. Check runtime dependencies before execution."
metadata:
  ...
  search-keywords: "long-video-to-clips, Long Video to Clip Candidates, 长视频高光候选, AI clip generator, long video to shorts, video highlight detector, podcast clips, 长视频切片, 高光切片, AI content automation, content automation, video automation, social media automation, 内容自动化"
  quality-score: "3.5/5"
---
# Long Video to Clip Candidates / 长视频高光候选

## Purpose / 用途

**中文**

分析长视频中的话题、语义密度和节奏，输出短视频候选片段，而非承诺预测爆款。

**English**

Analyze long-form video and produce short clip candidates based on topic boundaries, semantic density, and pacing.
...
`long-video-to-clips` · `Long Video to Clip Candidates` · `长视频高光候选` · `AI clip generator` · `long video to shorts` · `video highlight detector` · `podcast clips` · `长视频切片` · `高光切片` · `AI content automation` · `content automation` · `video automation` · `social media automation` · `内容自动化`

## When to use / 何时使用

Use this skill when the user explicitly requests **long video to clip candidates**, or when the requested outcome clearly matches this workflow.
...
```bash
npx skills add BryceYuuu/open-workflow-skills --skill long-video-to-clips
```

### Install for a specific supported agent

```bash
npx skills add BryceYuuu/open-workflow-skills --skill long-video-to-clips --agent codex
```
...
> 使用 `long-video-to-clips` 工作流处理我提供的输入。先检查依赖和权限，不覆盖源文件；执行后给我输出路径、验证结果和需要人工审核的部分。

**English**

> Use the `long-video-to-clips` workflow for the provided input. Check dependencies and permissions first, preserve the source, validate the result, and report outputs plus any human-review items.
