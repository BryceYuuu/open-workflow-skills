---
name: conditional-reminder
description: "Create reminders that depend on future conditions or external events using a scheduler plus the relevant data source or webhook. Use when the user asks for conditional reminder or an equivalent workflow."
license: MIT
compatibility: "Cross-agent Agent Skills format. Core stack: Scheduler / webhook or connector. Check runtime dependencies before execution."
metadata:
  ...
  automation-level: "L4"
  search-keywords: "conditional-reminder, Conditional Reminder, 条件式智能提醒, conditional reminder, smart reminder, event reminder, 条件提醒, 智能提醒, personal productivity, AI assistant, personal automation, 个人效率, AI助手, 个人自动化"
  quality-score: "4.0/5"
---
# Conditional Reminder / 条件式智能提醒

## Purpose / 用途

**中文**

使用调度器和对应数据源/Webhook 创建由未来条件触发的提醒，而不只是固定时间提醒。

**English**

Create reminders that depend on future conditions or external events using a scheduler plus the relevant data source or webhook.
...
`conditional-reminder` · `Conditional Reminder` · `条件式智能提醒` · `conditional reminder` · `smart reminder` · `event reminder` · `条件提醒` · `智能提醒` · `personal productivity` · `AI assistant` · `personal automation` · `个人效率` · `AI助手` · `个人自动化`

## When to use / 何时使用

Use this skill when the user explicitly requests **conditional reminder**, or when the requested outcome clearly matches this workflow.
...
`Scheduler / webhook or connector`
...
```bash
npx skills add BryceYuuu/open-workflow-skills --skill conditional-reminder
```

### Install for a specific supported agent

```bash
npx skills add BryceYuuu/open-workflow-skills --skill conditional-reminder --agent codex
```
...
> 使用 `conditional-reminder` 工作流处理我提供的输入。先检查依赖和权限，不覆盖源文件；执行后给我输出路径、验证结果和需要人工审核的部分。

**English**

> Use the `conditional-reminder` workflow for the provided input. Check dependencies and permissions first, preserve the source, validate the result, and report outputs plus any human-review items.
