---
name: speech-writer
description: "Turn structured ideas into speeches with narrative flow, timing, emphasis, and delivery notes. Use when the user asks for speech writer or an equivalent workflow."
license: MIT
...
metadata:
  ...
  quality-score: "4.5/5"
---
# Speech Writer / 演讲稿与演讲提示

## Purpose / 用途

**中文**

把要点转换成具有叙事结构的演讲稿，并加入时长、停顿、强调和演讲提示。

**English**

Turn structured ideas into speeches with narrative flow, timing, emphasis, and delivery notes.
...
`speech-writer` · `Speech Writer` · `演讲稿与演讲提示` · `AI speech writer` · `speech generator` · `presentation script` · `演讲稿生成` · `演讲助手` · `communication automation` · `social automation` · `AI writing assistant` · `沟通自动化` · `社交媒体自动化` · `AI写作`

## When to use / 何时使用

Use this skill when the user explicitly requests **speech writer**, or when the requested outcome clearly matches this workflow.
...
```bash
npx skills add BryceYuuu/open-workflow-skills --skill speech-writer
```

### Install for a specific supported agent

```bash
npx skills add BryceYuuu/open-workflow-skills --skill speech-writer --agent codex
```
...
> 使用 `speech-writer` 工作流处理我提供的输入。先检查依赖和权限，不覆盖源文件；执行后给我输出路径、验证结果和需要人工审核的部分。

**English**

> Use the `speech-writer` workflow for the provided input. Check dependencies and permissions first, preserve the source, validate the result, and report outputs plus any human-review items.
