...
description: "Explain difficult technical or academic material at a requested level using analogies, definitions, and worked examples. Use when the user asks for plain language explainer or an equivalent workflow."
license: MIT
...
metadata:
  ...
  status: "production"
  automation-level: "L1"
  search-keywords: "plain-language-explainer, Plain Language Explainer, 复杂知识讲人话, AI explainer, explain simply, ELI5, 复杂知识讲解, 通俗解释, AI research, learning automation, education AI, 研究助手, 学习自动化, AI教育"
  quality-score: "5.0/5"
---
# Plain Language Explainer / 复杂知识讲人话

## Purpose / 用途

**中文**

按指定理解层级把复杂技术或学术材料解释清楚，并使用定义、比喻和例子。

**English**

Explain difficult technical or academic material at a requested level using analogies, definitions, and worked examples.
...
`plain-language-explainer` · `Plain Language Explainer` · `复杂知识讲人话` · `AI explainer` · `explain simply` · `ELI5` · `复杂知识讲解` · `通俗解释` · `AI research` · `learning automation` · `education AI` · `研究助手` · `学习自动化` · `AI教育`

## When to use / 何时使用

Use this skill when the user explicitly requests **plain language explainer**, or when the requested outcome clearly matches this workflow.
...
```bash
npx skills add BryceYuuu/open-workflow-skills --skill plain-language-explainer
```

### Install for a specific supported agent

```bash
npx skills add BryceYuuu/open-workflow-skills --skill plain-language-explainer --agent codex
```
...
> 使用 `plain-language-explainer` 工作流处理我提供的输入。先检查依赖和权限，不覆盖源文件；执行后给我输出路径、验证结果和需要人工审核的部分。

**English**

> Use the `plain-language-explainer` workflow for the provided input. Check dependencies and permissions first, preserve the source, validate the result, and report outputs plus any human-review items.
