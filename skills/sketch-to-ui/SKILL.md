---
name: sketch-to-ui
description: "Interpret sketches or wireframes into structured high-fidelity UI while explicitly flagging missing interaction requirements. Use when the user asks for sketch to ui or an equivalent workflow."
license: MIT
compatibility: "Cross-agent Agent Skills format. Core stack: Vision model / UI generator. Check runtime dependencies before execution."
metadata:
  ...
  status: "experimental"
  automation-level: "L3"
  search-keywords: "sketch-to-ui, Sketch to UI, 草图转高保真 UI, sketch to UI, wireframe to UI, AI UI generator, 草图转UI, 线框图转设计, AI design, design automation, creative automation, AI设计, 设计自动化, 创意工作流"
  quality-score: "3.5/5"
---
# Sketch to UI / 草图转高保真 UI

## Purpose / 用途

**中文**

把草图或 Wireframe 转换成高保真 UI，并明确标出原图无法推断的交互需求。

**English**

Interpret sketches or wireframes into structured high-fidelity UI while explicitly flagging missing interaction requirements.
...
`sketch-to-ui` · `Sketch to UI` · `草图转高保真 UI` · `sketch to UI` · `wireframe to UI` · `AI UI generator` · `草图转UI` · `线框图转设计` · `AI design` · `design automation` · `creative automation` · `AI设计` · `设计自动化` · `创意工作流`

## When to use / 何时使用

Use this skill when the user explicitly requests **sketch to ui**, or when the requested outcome clearly matches this workflow.
...
```bash
npx skills add BryceYuuu/open-workflow-skills --skill sketch-to-ui
```

### Install for a specific supported agent

```bash
npx skills add BryceYuuu/open-workflow-skills --skill sketch-to-ui --agent codex
```
...
> 使用 `sketch-to-ui` 工作流处理我提供的输入。先检查依赖和权限，不覆盖源文件；执行后给我输出路径、验证结果和需要人工审核的部分。

**English**

> Use the `sketch-to-ui` workflow for the provided input. Check dependencies and permissions first, preserve the source, validate the result, and report outputs plus any human-review items.
