...
description: "Extract period-aware financial metrics, guidance, risk factors, and management commentary with source traceability. Use when the user asks for earnings analyzer or an equivalent workflow."
license: MIT
compatibility: "Cross-agent Agent Skills format. Core stack: PDF/web parser / calculations / LLM. Check runtime dependencies before execution."
metadata:
  ...
  search-keywords: "earnings-analyzer, Earnings Analyzer, 财报分析, earnings analysis, financial report summarizer, 10-K analysis, 财报总结, business automation, growth automation, AI business, 商业自动化, 增长自动化, AI商业"
  quality-score: "4.5/5"
---
...
从财报和 Earnings Call 提取带期间和单位的财务指标、指引、风险和管理层观点，并保持来源可追溯。

**English**

Extract period-aware financial metrics, guidance, risk factors, and management commentary with source traceability.
...
`earnings-analyzer` · `Earnings Analyzer` · `财报分析` · `earnings analysis` · `financial report summarizer` · `10-K analysis` · `财报总结` · `business automation` · `growth automation` · `AI business` · `商业自动化` · `增长自动化` · `AI商业`

## When to use / 何时使用

Use this skill when the user explicitly requests **earnings analyzer**, or when the requested outcome clearly matches this workflow.
...
`PDF/web parser / calculations / LLM`
...
```bash
npx skills add BryceYuuu/open-workflow-skills --skill earnings-analyzer
```

### Install for a specific supported agent

```bash
npx skills add BryceYuuu/open-workflow-skills --skill earnings-analyzer --agent codex
```
...
> 使用 `earnings-analyzer` 工作流处理我提供的输入。先检查依赖和权限，不覆盖源文件；执行后给我输出路径、验证结果和需要人工审核的部分。

**English**

> Use the `earnings-analyzer` workflow for the provided input. Check dependencies and permissions first, preserve the source, validate the result, and report outputs plus any human-review items.
