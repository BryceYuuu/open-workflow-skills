...
description: "Batch-add logo or text watermarks to image folders without overwriting source files. Use when the user asks for image watermark or an equivalent workflow."
license: MIT
compatibility: "Cross-agent Agent Skills format. Core stack: ImageMagick / Pillow. Check runtime dependencies before execution."
metadata:
  ...
  automation-level: "L2"
  search-keywords: "image-watermark, Image Watermark, 批量图片水印, batch watermark, add watermark to images, photo watermark, image batch processing, 图片批量加水印, 图片加logo, AI content automation, content automation, video automation, social media automation, 内容自动化"
  quality-score: "5.0/5"
---
# Image Watermark / 批量图片水印

## Purpose / 用途

**中文**

批量读取图片并添加 Logo、文字水印或版权信息，默认不覆盖源文件。

**English**

Batch-add logo or text watermarks to image folders without overwriting source files.
...
`image-watermark` · `Image Watermark` · `批量图片水印` · `batch watermark` · `add watermark to images` · `photo watermark` · `image batch processing` · `图片批量加水印` · `图片加logo` · `AI content automation` · `content automation` · `video automation` · `social media automation` · `内容自动化`

## When to use / 何时使用

Use this skill when the user explicitly requests **image watermark**, or when the requested outcome clearly matches this workflow.
...
- Processing manifest / error report
...
2. **Validate source formats and preserve source hashes**
3. **Load watermark and calculate relative scale/position**
4. **Render copies with Pillow/ImageMagick-compatible logic**
5. **Write output manifest**
6. **Verify output count and dimensions**
...
ows run image-watermark -- --help
ows test image-watermark
```
...
```bash
npx skills add BryceYuuu/open-workflow-skills --skill image-watermark
```

### Install for a specific supported agent

```bash
npx skills add BryceYuuu/open-workflow-skills --skill image-watermark --agent codex
```
...
> 使用 `image-watermark` 工作流处理我提供的输入。先检查依赖和权限，不覆盖源文件；执行后给我输出路径、验证结果和需要人工审核的部分。

**English**

> Use the `image-watermark` workflow for the provided input. Check dependencies and permissions first, preserve the source, validate the result, and report outputs plus any human-review items.
