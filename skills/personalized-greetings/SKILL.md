...
description: "Generate personalized greeting drafts from user-provided relationship and context data without inventing private facts. Use when the user asks for personalized greetings or an equivalent workflow."
license: MIT
...
metadata:
  ...
  search-keywords: "personalized-greetings, Personalized Greetings, 个性化祝福, AI greeting generator, personalized messages, 祝福语生成, communication automation, social automation, AI writing assistant, 沟通自动化, 社交媒体自动化, AI写作"
  quality-score: "4.0/5"
---
# Personalized Greetings / 个性化祝福

## Purpose / 用途

**中文**

根据用户提供的关系和背景生成个性化祝福草稿，不编造私人信息。

**English**

Generate personalized greeting drafts from user-provided relationship and context data without inventing private facts.
...
`personalized-greetings` · `Personalized Greetings` · `个性化祝福` · `AI greeting generator` · `personalized messages` · `祝福语生成` · `communication automation` · `social automation` · `AI writing assistant` · `沟通自动化` · `社交媒体自动化` · `AI写作`

## When to use / 何时使用

Use this skill when the user explicitly requests **personalized greetings**, or when the requested outcome clearly matches this workflow.
...
```bash
npx skills add BryceYuuu/open-workflow-skills --skill personalized-greetings
```

### Install for a specific supported agent

```bash
npx skills add BryceYuuu/open-workflow-skills --skill personalized-greetings --agent codex
```
...
> 使用 `personalized-greetings` 工作流处理我提供的输入。先检查依赖和权限，不覆盖源文件；执行后给我输出路径、验证结果和需要人工审核的部分。

**English**

> Use the `personalized-greetings` workflow for the provided input. Check dependencies and permissions first, preserve the source, validate the result, and report outputs plus any human-review items.
