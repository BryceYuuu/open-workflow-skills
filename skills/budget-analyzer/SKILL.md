...
description: "Classify transaction data and analyze recurring spend, income, cash flow, subscriptions, and budget variance. Use when the user asks for budget analyzer or an equivalent workflow."
license: MIT
compatibility: "Cross-agent Agent Skills format. Core stack: CSV/financial connector / Python. Check runtime dependencies before execution."
metadata:
  ...
  search-keywords: "budget-analyzer, Budget Analyzer, 预算与现金流分析, AI budget planner, expense analysis, cash flow analysis, 预算分析, 消费分析, business automation, growth automation, AI business, 商业自动化, 增长自动化, AI商业"
  quality-score: "4.5/5"
---
# Budget Analyzer / 预算与现金流分析

## Purpose / 用途

**中文**

分类流水并分析固定支出、收入、现金流、订阅和预算偏差；接入真实账户时需严格权限控制。

**English**

Classify transaction data and analyze recurring spend, income, cash flow, subscriptions, and budget variance.
...
`budget-analyzer` · `Budget Analyzer` · `预算与现金流分析` · `AI budget planner` · `expense analysis` · `cash flow analysis` · `预算分析` · `消费分析` · `business automation` · `growth automation` · `AI business` · `商业自动化` · `增长自动化` · `AI商业`

## When to use / 何时使用

Use this skill when the user explicitly requests **budget analyzer**, or when the requested outcome clearly matches this workflow.
...
`CSV/financial connector / Python`
...
```bash
npx skills add BryceYuuu/open-workflow-skills --skill budget-analyzer
```

### Install for a specific supported agent

```bash
npx skills add BryceYuuu/open-workflow-skills --skill budget-analyzer --agent codex
```
...
> 使用 `budget-analyzer` 工作流处理我提供的输入。先检查依赖和权限，不覆盖源文件；执行后给我输出路径、验证结果和需要人工审核的部分。

**English**

> Use the `budget-analyzer` workflow for the provided input. Check dependencies and permissions first, preserve the source, validate the result, and report outputs plus any human-review items.
