---
name: product-mockup
description: "Generate product presentation mockups from approved source assets, favoring deterministic templates when exact geometry or text fidelity matters. Use when the user asks for product mockup generator or an equivalent workflow."
license: MIT
compatibility: "Cross-agent Agent Skills format. Core stack: Image tools / mockup templates / optional image API. Check runtime dependencies before execution."
metadata:
  ...
  status: "experimental"
  automation-level: "L3"
  search-keywords: "product-mockup, Product Mockup Generator, 产品 Mockup 生成, AI mockup generator, product mockup, packaging mockup, 产品样机, 产品效果图, AI design, design automation, creative automation, AI设计, 设计自动化, 创意工作流"
  quality-score: "3.0/5"
---
# Product Mockup Generator / 产品 Mockup 生成

## Purpose / 用途

**中文**

根据产品素材生成展示 Mockup；对精确几何、Logo 和文字要求高时优先模板而非纯生成图。

**English**

Generate product presentation mockups from approved source assets, favoring deterministic templates when exact geometry or text fidelity matters.
...
`product-mockup` · `Product Mockup Generator` · `产品 Mockup 生成` · `AI mockup generator` · `product mockup` · `packaging mockup` · `产品样机` · `产品效果图` · `AI design` · `design automation` · `creative automation` · `AI设计` · `设计自动化` · `创意工作流`

## When to use / 何时使用

Use this skill when the user explicitly requests **product mockup generator**, or when the requested outcome clearly matches this workflow.
...
`Image tools / mockup templates / optional image API`
...
```bash
npx skills add BryceYuuu/open-workflow-skills --skill product-mockup
```

### Install for a specific supported agent

```bash
npx skills add BryceYuuu/open-workflow-skills --skill product-mockup --agent codex
```
...
> 使用 `product-mockup` 工作流处理我提供的输入。先检查依赖和权限，不覆盖源文件；执行后给我输出路径、验证结果和需要人工审核的部分。

**English**

> Use the `product-mockup` workflow for the provided input. Check dependencies and permissions first, preserve the source, validate the result, and report outputs plus any human-review items.
