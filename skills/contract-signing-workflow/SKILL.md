...
description: "Track contract status, route documents, and send reminders while keeping legal judgment and final signing under human control. Use when the user asks for contract signing workflow or an equivalent workflow."
license: MIT
compatibility: "Cross-agent Agent Skills format. Core stack: E-sign API / document storage. Check runtime dependencies before execution."
metadata:
  ...
  search-keywords: "contract-signing-workflow, Contract Signing Workflow, 合同签署流转, contract workflow, e-signature workflow, signature reminder, 合同催签, 电子签流程, office automation, AI productivity, workflow automation, 办公自动化, 效率工具, AI办公"
  quality-score: "3.5/5"
---
# Contract Signing Workflow / 合同签署流转

## Purpose / 用途

**中文**

负责合同状态、流转和催签；法律判断、最终发送与签署必须保留人工控制。

**English**

Track contract status, route documents, and send reminders while keeping legal judgment and final signing under human control.
...
`contract-signing-workflow` · `Contract Signing Workflow` · `合同签署流转` · `contract workflow` · `e-signature workflow` · `signature reminder` · `合同催签` · `电子签流程` · `office automation` · `AI productivity` · `workflow automation` · `办公自动化` · `效率工具` · `AI办公`

## When to use / 何时使用

Use this skill when the user explicitly requests **contract signing workflow**, or when the requested outcome clearly matches this workflow.
...
```bash
npx skills add BryceYuuu/open-workflow-skills --skill contract-signing-workflow
```

### Install for a specific supported agent

```bash
npx skills add BryceYuuu/open-workflow-skills --skill contract-signing-workflow --agent codex
```
...
> 使用 `contract-signing-workflow` 工作流处理我提供的输入。先检查依赖和权限，不覆盖源文件；执行后给我输出路径、验证结果和需要人工审核的部分。

**English**

> Use the `contract-signing-workflow` workflow for the provided input. Check dependencies and permissions first, preserve the source, validate the result, and report outputs plus any human-review items.
