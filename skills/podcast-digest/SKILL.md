---
name: podcast-digest
description: "Process RSS feeds, episode URLs, or local audio into transcripts, chapters, summaries, key ideas, and references. Use when the user asks for podcast digest or an equivalent workflow."
license: MIT
compatibility: "Cross-agent Agent Skills format. Core stack: RSS parser / ASR / LLM. Check runtime dependencies before execution."
metadata:
  ...
  search-keywords: "podcast-digest, Podcast Digest, 播客摘要与文字稿, podcast summarizer, podcast transcript, RSS podcast summary, 播客总结, 播客转文字, AI content automation, content automation, video automation, social media automation, 内容自动化, 视频自动化"
  quality-score: "4.5/5"
---
# Podcast Digest / 播客摘要与文字稿

## Purpose / 用途

**中文**

把 RSS、单集链接或本地音频转换成文字稿、章节、摘要、核心观点和引用信息。

**English**

Process RSS feeds, episode URLs, or local audio into transcripts, chapters, summaries, key ideas, and references.
...
`podcast-digest` · `Podcast Digest` · `播客摘要与文字稿` · `podcast summarizer` · `podcast transcript` · `RSS podcast summary` · `播客总结` · `播客转文字` · `AI content automation` · `content automation` · `video automation` · `social media automation` · `内容自动化` · `视频自动化`

## When to use / 何时使用

Use this skill when the user explicitly requests **podcast digest**, or when the requested outcome clearly matches this workflow.
...
1. **Load a transcript or latest RSS episode metadata**
2. **Normalize source text**
3. **Extract concise key points**
4. **Write digest.md and metadata.json**
5. **Verify source metadata and output files**
...
```bash
ows doctor podcast-digest
ows run podcast-digest -- --help
ows test podcast-digest
```
...
```bash
npx skills add BryceYuuu/open-workflow-skills --skill podcast-digest
```

### Install for a specific supported agent

```bash
npx skills add BryceYuuu/open-workflow-skills --skill podcast-digest --agent codex
```
...
> 使用 `podcast-digest` 工作流处理我提供的输入。先检查依赖和权限，不覆盖源文件；执行后给我输出路径、验证结果和需要人工审核的部分。

**English**

> Use the `podcast-digest` workflow for the provided input. Check dependencies and permissions first, preserve the source, validate the result, and report outputs plus any human-review items.
