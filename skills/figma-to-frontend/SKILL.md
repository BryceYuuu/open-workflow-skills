...
description: "Use authorized Figma design context to generate component structure, assets, responsive layouts, and frontend code with visual validation. Use when the user asks for figma to frontend or an equivalent workflow."
license: MIT
...
metadata:
  ...
  quality-score: "4.0/5"
---
# Figma to Frontend / Figma 转前端

## Purpose / 用途

**中文**

通过授权的 Figma 设计上下文生成组件结构、资产、响应式布局和前端代码，并进行视觉验证。

**English**

Use authorized Figma design context to generate component structure, assets, responsive layouts, and frontend code with visual validation.
...
`figma-to-frontend` · `Figma to Frontend` · `Figma 转前端` · `Figma to code` · `Figma to React` · `design to code` · `Figma转代码` · `设计稿转前端`

## When to use / 何时使用

Use this skill when the user explicitly requests **figma to frontend**, or when the requested outcome clearly matches this workflow.
...
`Figma API/MCP / frontend stack / browser`
...
```bash
npx skills add BryceYuuu/open-workflow-skills --skill figma-to-frontend
```

### Install for a specific supported agent

```bash
npx skills add BryceYuuu/open-workflow-skills --skill figma-to-frontend --agent codex
```
...
> 使用 `figma-to-frontend` 工作流处理我提供的输入。先检查依赖和权限，不覆盖源文件；执行后给我输出路径、验证结果和需要人工审核的部分。

**English**

> Use the `figma-to-frontend` workflow for the provided input. Check dependencies and permissions first, preserve the source, validate the result, and report outputs plus any human-review items.
