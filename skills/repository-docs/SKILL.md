...
description: "Scan a repository and generate or update README, architecture, setup, environment, API, deployment, and troubleshooting docs. Use when the user asks for repository documentation or an equivalent workflow."
license: MIT
compatibility: "Cross-agent Agent Skills format. Core stack: Git / repository tools. Check runtime dependencies before execution."
metadata:
  ...
  automation-level: "L2"
  search-keywords: "repository-docs, Repository Documentation, 代码仓库文档生成, AI README generator, code documentation, API documentation generator, README生成, 代码文档"
  quality-score: "4.5/5"
---
# Repository Documentation / 代码仓库文档生成

## Purpose / 用途

**中文**

扫描代码仓库并生成或更新 README、架构、安装、环境变量、API、部署和排障文档。

**English**

Scan a repository and generate or update README, architecture, setup, environment, API, deployment, and troubleshooting docs.
...
`repository-docs` · `Repository Documentation` · `代码仓库文档生成` · `AI README generator` · `code documentation` · `API documentation generator` · `README生成` · `代码文档`

## When to use / 何时使用

Use this skill when the user explicitly requests **repository documentation**, or when the requested outcome clearly matches this workflow.
...
1. **Walk repository while excluding generated/vendor directories**
2. **Count file types**
3. **Build bounded-depth project tree**
4. **Generate repository overview Markdown**
5. **Write machine-readable statistics**
...
```bash
ows doctor repository-docs
ows run repository-docs -- --help
ows test repository-docs
```
...
```bash
npx skills add BryceYuuu/open-workflow-skills --skill repository-docs
```

### Install for a specific supported agent

```bash
npx skills add BryceYuuu/open-workflow-skills --skill repository-docs --agent codex
```
...
> 使用 `repository-docs` 工作流处理我提供的输入。先检查依赖和权限，不覆盖源文件；执行后给我输出路径、验证结果和需要人工审核的部分。

**English**

> Use the `repository-docs` workflow for the provided input. Check dependencies and permissions first, preserve the source, validate the result, and report outputs plus any human-review items.
