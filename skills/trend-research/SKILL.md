---
name: trend-research
description: "Collect dated public signals across search, news, products, repositories, and social sources and produce a sourced trend report. Use when the user asks for trend research agent or an equivalent workflow."
license: MIT
compatibility: "Cross-agent Agent Skills format. Core stack: Search / browser / source APIs / LLM. Check runtime dependencies before execution."
metadata:
  ...
  search-keywords: "trend-research, Trend Research Agent, 趋势研究 Agent, AI trend research, market trend analysis, trend monitoring, 趋势研究, 热点分析, data automation, AI data analysis, Excel automation, analytics workflow, 数据自动化, 数据分析"
  quality-score: "3.5/5"
---
# Trend Research Agent / 趋势研究 Agent

## Purpose / 用途

**中文**

跨搜索、新闻、产品、代码仓库和社交来源收集带日期的公开信号，生成可追溯趋势报告。

**English**

Collect dated public signals across search, news, products, repositories, and social sources and produce a sourced trend report.
...
`trend-research` · `Trend Research Agent` · `趋势研究 Agent` · `AI trend research` · `market trend analysis` · `trend monitoring` · `趋势研究` · `热点分析` · `data automation` · `AI data analysis` · `Excel automation` · `analytics workflow` · `数据自动化` · `数据分析`

## When to use / 何时使用

Use this skill when the user explicitly requests **trend research agent**, or when the requested outcome clearly matches this workflow.
...
`Search / browser / source APIs / LLM`
...
```bash
npx skills add BryceYuuu/open-workflow-skills --skill trend-research
```

### Install for a specific supported agent

```bash
npx skills add BryceYuuu/open-workflow-skills --skill trend-research --agent codex
```
...
> 使用 `trend-research` 工作流处理我提供的输入。先检查依赖和权限，不覆盖源文件；执行后给我输出路径、验证结果和需要人工审核的部分。

**English**

> Use the `trend-research` workflow for the provided input. Check dependencies and permissions first, preserve the source, validate the result, and report outputs plus any human-review items.
