...
description: "Generate role-specific onboarding packs and optionally orchestrate account or task setup through connected systems. Use when the user asks for employee onboarding or an equivalent workflow."
license: MIT
compatibility: "Cross-agent Agent Skills format. Core stack: LLM / HRIS or task integrations. Check runtime dependencies before execution."
metadata:
  ...
  search-keywords: "employee-onboarding, Employee Onboarding, 新人入职资料生成, AI onboarding, employee onboarding automation, new hire training, 员工入职, 新人培训, office automation, AI productivity, workflow automation, 办公自动化, 效率工具, AI办公"
  quality-score: "4.0/5"
---
# Employee Onboarding / 新人入职资料生成

## Purpose / 用途

**中文**

根据岗位生成新人资料包、培训和 30/60/90 计划；创建账号和权限需 HR/IT 系统连接。

**English**

Generate role-specific onboarding packs and optionally orchestrate account or task setup through connected systems.
...
`employee-onboarding` · `Employee Onboarding` · `新人入职资料生成` · `AI onboarding` · `employee onboarding automation` · `new hire training` · `员工入职` · `新人培训` · `office automation` · `AI productivity` · `workflow automation` · `办公自动化` · `效率工具` · `AI办公`

## When to use / 何时使用

Use this skill when the user explicitly requests **employee onboarding**, or when the requested outcome clearly matches this workflow.
...
```bash
npx skills add BryceYuuu/open-workflow-skills --skill employee-onboarding
```

### Install for a specific supported agent

```bash
npx skills add BryceYuuu/open-workflow-skills --skill employee-onboarding --agent codex
```
...
> 使用 `employee-onboarding` 工作流处理我提供的输入。先检查依赖和权限，不覆盖源文件；执行后给我输出路径、验证结果和需要人工审核的部分。

**English**

> Use the `employee-onboarding` workflow for the provided input. Check dependencies and permissions first, preserve the source, validate the result, and report outputs plus any human-review items.
