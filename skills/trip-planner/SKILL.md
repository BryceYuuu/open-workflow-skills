---
name: trip-planner
description: "Build practical itineraries from destination, time, budget, transport, and preferences using fresh location and availability data when connected. Use when the user asks for trip planner or an equivalent workflow."
license: MIT
...
metadata:
  ...
  quality-score: "4.0/5"
---
# Trip Planner / 旅行规划 Agent

## Purpose / 用途

**中文**

根据目的地、时间、预算、交通和偏好生成实际行程；高质量结果应连接最新地点、营业时间和交通信息。

**English**

Build practical itineraries from destination, time, budget, transport, and preferences using fresh location and availability data when connected.
...
`trip-planner` · `Trip Planner` · `旅行规划 Agent` · `AI trip planner` · `travel itinerary` · `travel planning` · `AI旅行规划` · `行程生成` · `personal productivity` · `AI assistant` · `personal automation` · `个人效率` · `AI助手` · `个人自动化`

## When to use / 何时使用

Use this skill when the user explicitly requests **trip planner**, or when the requested outcome clearly matches this workflow.
...
```bash
npx skills add BryceYuuu/open-workflow-skills --skill trip-planner
```

### Install for a specific supported agent

```bash
npx skills add BryceYuuu/open-workflow-skills --skill trip-planner --agent codex
```
...
> 使用 `trip-planner` 工作流处理我提供的输入。先检查依赖和权限，不覆盖源文件；执行后给我输出路径、验证结果和需要人工审核的部分。

**English**

> Use the `trip-planner` workflow for the provided input. Check dependencies and permissions first, preserve the source, validate the result, and report outputs plus any human-review items.
