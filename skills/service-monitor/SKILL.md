...
description: "Deploy or configure health checks for endpoints, processes, ports, and resource metrics and route alerts to configured channels. Use when the user asks for service monitor or an equivalent workflow."
license: MIT
compatibility: "Cross-agent Agent Skills format. Core stack: Monitoring stack / scheduler / alerting. Check runtime dependencies before execution."
metadata:
  ...
  status: "production"
  automation-level: "L4"
  search-keywords: "service-monitor, Service Monitor, 服务健康监控, service monitoring, uptime monitoring, health check, 服务监控, 网站监控, DevOps automation, system automation, infrastructure automation, DevOps自动化, 运维自动化, 系统自动化"
  quality-score: "4.5/5"
---
# Service Monitor / 服务健康监控

## Purpose / 用途

**中文**

配置网站、API、进程、端口和资源指标健康检查，并把异常发送到指定通知渠道。

**English**

Deploy or configure health checks for endpoints, processes, ports, and resource metrics and route alerts to configured channels.
...
`service-monitor` · `Service Monitor` · `服务健康监控` · `service monitoring` · `uptime monitoring` · `health check` · `服务监控` · `网站监控` · `DevOps automation` · `system automation` · `infrastructure automation` · `DevOps自动化` · `运维自动化` · `系统自动化`

## When to use / 何时使用

Use this skill when the user explicitly requests **service monitor**, or when the requested outcome clearly matches this workflow.
...
`Monitoring stack / scheduler / alerting`
...
```bash
npx skills add BryceYuuu/open-workflow-skills --skill service-monitor
```

### Install for a specific supported agent

```bash
npx skills add BryceYuuu/open-workflow-skills --skill service-monitor --agent codex
```
...
> 使用 `service-monitor` 工作流处理我提供的输入。先检查依赖和权限，不覆盖源文件；执行后给我输出路径、验证结果和需要人工审核的部分。

**English**

> Use the `service-monitor` workflow for the provided input. Check dependencies and permissions first, preserve the source, validate the result, and report outputs plus any human-review items.
