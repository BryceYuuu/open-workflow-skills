...
description: "Batch-extract structured fields from invoices, receipts, screenshots, or PDFs and export validated JSON, CSV, or Excel. Use when the user asks for invoice data extractor or an equivalent workflow."
license: MIT
compatibility: "Cross-agent Agent Skills format. Core stack: Vision/OCR / Python / Pandas. Check runtime dependencies before execution."
metadata:
  ...
  search-keywords: "invoice-data-extractor, Invoice Data Extractor, 发票数据提取, invoice OCR, receipt data extraction, invoice to Excel, 发票提取, 发票转Excel, data automation, AI data analysis, Excel automation, analytics workflow, 数据自动化, 数据分析"
  quality-score: "4.5/5"
---
# Invoice Data Extractor / 发票数据提取

## Purpose / 用途

**中文**

批量提取发票、收据、截图或 PDF 的商户、日期、金额、税额、币种和明细，输出经过校验的结构化数据。

**English**

Batch-extract structured fields from invoices, receipts, screenshots, or PDFs and export validated JSON, CSV, or Excel.
...
`invoice-data-extractor` · `Invoice Data Extractor` · `发票数据提取` · `invoice OCR` · `receipt data extraction` · `invoice to Excel` · `发票提取` · `发票转Excel` · `data automation` · `AI data analysis` · `Excel automation` · `analytics workflow` · `数据自动化` · `数据分析`

## When to use / 何时使用

Use this skill when the user explicitly requests **invoice data extractor**, or when the requested outcome clearly matches this workflow.
...
1. **Load text directly or OCR via optional Tesseract**
2. **Extract vendor, invoice number, date and total**
3. **Normalize numeric amounts**
4. **Write structured JSON**
5. **Leave missing fields null rather than guessing**
...
```bash
ows doctor invoice-data-extractor
ows run invoice-data-extractor -- --help
ows test invoice-data-extractor
```
...
```bash
npx skills add BryceYuuu/open-workflow-skills --skill invoice-data-extractor
```

### Install for a specific supported agent

```bash
npx skills add BryceYuuu/open-workflow-skills --skill invoice-data-extractor --agent codex
```
...
> 使用 `invoice-data-extractor` 工作流处理我提供的输入。先检查依赖和权限，不覆盖源文件；执行后给我输出路径、验证结果和需要人工审核的部分。

**English**

> Use the `invoice-data-extractor` workflow for the provided input. Check dependencies and permissions first, preserve the source, validate the result, and report outputs plus any human-review items.
