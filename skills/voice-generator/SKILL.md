...
description: "Turn scripts into controlled speech with pacing, pause, and style settings using an authorized TTS provider. Use when the user asks for voice generator or an equivalent workflow."
license: MIT
compatibility: "Cross-agent Agent Skills format. Core stack: TTS API. Check runtime dependencies before execution."
metadata:
  ...
  automation-level: "L3"
  search-keywords: "voice-generator, Voice Generator, AI 配音生成, AI voice generator, text to speech, TTS, voiceover generator, AI配音, 文字转语音, AI content automation, content automation, video automation, social media automation, 内容自动化"
  quality-score: "4.0/5"
---
# Voice Generator / AI 配音生成

## Purpose / 用途

**中文**

通过合法授权的 TTS 服务把脚本转成可控语速、停顿和风格的配音。

**English**

Turn scripts into controlled speech with pacing, pause, and style settings using an authorized TTS provider.
...
`voice-generator` · `Voice Generator` · `AI 配音生成` · `AI voice generator` · `text to speech` · `TTS` · `voiceover generator` · `AI配音` · `文字转语音` · `AI content automation` · `content automation` · `video automation` · `social media automation` · `内容自动化`

## When to use / 何时使用

Use this skill when the user explicitly requests **voice generator**, or when the requested outcome clearly matches this workflow.
...
```bash
npx skills add BryceYuuu/open-workflow-skills --skill voice-generator
```

### Install for a specific supported agent

```bash
npx skills add BryceYuuu/open-workflow-skills --skill voice-generator --agent codex
```
...
> 使用 `voice-generator` 工作流处理我提供的输入。先检查依赖和权限，不覆盖源文件；执行后给我输出路径、验证结果和需要人工审核的部分。

**English**

> Use the `voice-generator` workflow for the provided input. Check dependencies and permissions first, preserve the source, validate the result, and report outputs plus any human-review items.
