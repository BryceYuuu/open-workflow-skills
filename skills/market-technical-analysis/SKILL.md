...
description: "Calculate technical indicators and describe observed market conditions without presenting guaranteed buy or sell outcomes. Use when the user asks for market technical analysis or an equivalent workflow."
license: MIT
compatibility: "Cross-agent Agent Skills format. Core stack: Market data / Python. Check runtime dependencies before execution."
metadata:
  ...
  search-keywords: "market-technical-analysis, Market Technical Analysis, 市场技术指标分析, technical analysis AI, market indicators, stock chart analysis, 技术分析, K线分析, business automation, growth automation, AI business, 商业自动化, 增长自动化, AI商业"
  quality-score: "4.0/5"
---
# Market Technical Analysis / 市场技术指标分析

## Purpose / 用途

**中文**

计算技术指标并描述当前市场状态，不输出保证性买卖结论。

**English**

Calculate technical indicators and describe observed market conditions without presenting guaranteed buy or sell outcomes.
...
`market-technical-analysis` · `Market Technical Analysis` · `市场技术指标分析` · `technical analysis AI` · `market indicators` · `stock chart analysis` · `技术分析` · `K线分析` · `business automation` · `growth automation` · `AI business` · `商业自动化` · `增长自动化` · `AI商业`

## When to use / 何时使用

Use this skill when the user explicitly requests **market technical analysis**, or when the requested outcome clearly matches this workflow.
...
```bash
npx skills add BryceYuuu/open-workflow-skills --skill market-technical-analysis
```

### Install for a specific supported agent

```bash
npx skills add BryceYuuu/open-workflow-skills --skill market-technical-analysis --agent codex
```
...
> 使用 `market-technical-analysis` 工作流处理我提供的输入。先检查依赖和权限，不覆盖源文件；执行后给我输出路径、验证结果和需要人工审核的部分。

**English**

> Use the `market-technical-analysis` workflow for the provided input. Check dependencies and permissions first, preserve the source, validate the result, and report outputs plus any human-review items.
...
- Treat outputs as analysis, not guaranteed financial outcomes; never execute live trades by default.
