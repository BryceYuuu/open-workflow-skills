...
description: "Process long books chapter by chapter and build a structured map of concepts, arguments, evidence, and cross-chapter relationships. Use when the user asks for book knowledge map or an equivalent workflow."
license: MIT
compatibility: "Cross-agent Agent Skills format. Core stack: Document parser / LLM. Check runtime dependencies before execution."
metadata:
  ...
  search-keywords: "book-knowledge-map, Book Knowledge Map, 书籍知识地图, book summarizer, book knowledge graph, book analysis, 书籍总结, 知识地图, AI research, learning automation, education AI, 研究助手, 学习自动化, AI教育"
  quality-score: "4.0/5"
---
# Book Knowledge Map / 书籍知识地图

## Purpose / 用途

**中文**

按章节处理长书，构建概念、论点、证据及跨章节关系的结构化知识地图。

**English**

Process long books chapter by chapter and build a structured map of concepts, arguments, evidence, and cross-chapter relationships.
...
`book-knowledge-map` · `Book Knowledge Map` · `书籍知识地图` · `book summarizer` · `book knowledge graph` · `book analysis` · `书籍总结` · `知识地图` · `AI research` · `learning automation` · `education AI` · `研究助手` · `学习自动化` · `AI教育`

## When to use / 何时使用

Use this skill when the user explicitly requests **book knowledge map**, or when the requested outcome clearly matches this workflow.
...
```bash
npx skills add BryceYuuu/open-workflow-skills --skill book-knowledge-map
```

### Install for a specific supported agent

```bash
npx skills add BryceYuuu/open-workflow-skills --skill book-knowledge-map --agent codex
```
...
> 使用 `book-knowledge-map` 工作流处理我提供的输入。先检查依赖和权限，不覆盖源文件；执行后给我输出路径、验证结果和需要人工审核的部分。

**English**

> Use the `book-knowledge-map` workflow for the provided input. Check dependencies and permissions first, preserve the source, validate the result, and report outputs plus any human-review items.
