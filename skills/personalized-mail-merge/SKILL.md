---
name: personalized-mail-merge
description: "Generate personalized outreach drafts from a spreadsheet and optionally send them after human approval. Use when the user asks for personalized mail merge or an equivalent workflow."
license: MIT
compatibility: "Cross-agent Agent Skills format. Core stack: CSV/Excel / LLM / Gmail or SMTP. Check runtime dependencies before execution."
metadata:
  ...
  search-keywords: "personalized-mail-merge, Personalized Mail Merge, 个性化批量邮件, AI mail merge, personalized cold email, bulk personalized email, 批量个性化邮件, 邮件群发, office automation, AI productivity, workflow automation, 办公自动化, 效率工具, AI办公"
  quality-score: "4.0/5"
---
# Personalized Mail Merge / 个性化批量邮件

## Purpose / 用途

**中文**

根据表格联系人信息批量生成差异化邮件草稿；真正发送需要邮箱授权并默认人工批准。

**English**

Generate personalized outreach drafts from a spreadsheet and optionally send them after human approval.
...
`personalized-mail-merge` · `Personalized Mail Merge` · `个性化批量邮件` · `AI mail merge` · `personalized cold email` · `bulk personalized email` · `批量个性化邮件` · `邮件群发` · `office automation` · `AI productivity` · `workflow automation` · `办公自动化` · `效率工具` · `AI办公`

## When to use / 何时使用

Use this skill when the user explicitly requests **personalized mail merge**, or when the requested outcome clearly matches this workflow.
...
```bash
npx skills add BryceYuuu/open-workflow-skills --skill personalized-mail-merge
```

### Install for a specific supported agent

```bash
npx skills add BryceYuuu/open-workflow-skills --skill personalized-mail-merge --agent codex
```
...
> 使用 `personalized-mail-merge` 工作流处理我提供的输入。先检查依赖和权限，不覆盖源文件；执行后给我输出路径、验证结果和需要人工审核的部分。

**English**

> Use the `personalized-mail-merge` workflow for the provided input. Check dependencies and permissions first, preserve the source, validate the result, and report outputs plus any human-review items.
