...
description: "Summarize academic papers into research question, method, dataset, findings, contributions, limitations, and open questions. Use when the user asks for paper summarizer or an equivalent workflow."
license: MIT
compatibility: "Cross-agent Agent Skills format. Core stack: PDF parser / LLM. Check runtime dependencies before execution."
metadata:
  ...
  search-keywords: "paper-summarizer, Paper Summarizer, 论文深度解读, AI paper summarizer, research paper summary, PDF paper analysis, 论文总结, 论文解读, AI research, learning automation, education AI, 研究助手, 学习自动化, AI教育"
  quality-score: "4.5/5"
---
# Paper Summarizer / 论文深度解读

## Purpose / 用途

**中文**

把论文整理成研究问题、方法、数据、结果、创新、局限和开放问题。

**English**

Summarize academic papers into research question, method, dataset, findings, contributions, limitations, and open questions.
...
`paper-summarizer` · `Paper Summarizer` · `论文深度解读` · `AI paper summarizer` · `research paper summary` · `PDF paper analysis` · `论文总结` · `论文解读` · `AI research` · `learning automation` · `education AI` · `研究助手` · `学习自动化` · `AI教育`

## When to use / 何时使用

Use this skill when the user explicitly requests **paper summarizer**, or when the requested outcome clearly matches this workflow.
...
```bash
npx skills add BryceYuuu/open-workflow-skills --skill paper-summarizer
```

### Install for a specific supported agent

```bash
npx skills add BryceYuuu/open-workflow-skills --skill paper-summarizer --agent codex
```
...
> 使用 `paper-summarizer` 工作流处理我提供的输入。先检查依赖和权限，不覆盖源文件；执行后给我输出路径、验证结果和需要人工审核的部分。

**English**

> Use the `paper-summarizer` workflow for the provided input. Check dependencies and permissions first, preserve the source, validate the result, and report outputs plus any human-review items.
