...
description: "Convert an article into a script, storyboard, voice track, asset plan, and render candidates. Use when the user asks for article to video or an equivalent workflow."
license: MIT
...
metadata:
  ...
  automation-level: "L3"
  search-keywords: "article-to-video, Article to Video, 文章转视频, blog to video, article video generator, text to video workflow, 图文转视频, AI content automation, content automation, video automation, social media automation, 内容自动化, 视频自动化, 自媒体自动化"
  quality-score: "3.0/5"
---
# Article to Video / 文章转视频

## Purpose / 用途

**中文**

把文章转换为脚本、分镜、配音和素材计划，并生成成片候选；最终剪辑质量需人工确认。

**English**

Convert an article into a script, storyboard, voice track, asset plan, and render candidates.
...
`article-to-video` · `Article to Video` · `文章转视频` · `blog to video` · `article video generator` · `text to video workflow` · `图文转视频` · `AI content automation` · `content automation` · `video automation` · `social media automation` · `内容自动化` · `视频自动化` · `自媒体自动化`

## When to use / 何时使用

Use this skill when the user explicitly requests **article to video**, or when the requested outcome clearly matches this workflow.
...
```bash
npx skills add BryceYuuu/open-workflow-skills --skill article-to-video
```

### Install for a specific supported agent

```bash
npx skills add BryceYuuu/open-workflow-skills --skill article-to-video --agent codex
```
...
> 使用 `article-to-video` 工作流处理我提供的输入。先检查依赖和权限，不覆盖源文件；执行后给我输出路径、验证结果和需要人工审核的部分。

**English**

> Use the `article-to-video` workflow for the provided input. Check dependencies and permissions first, preserve the source, validate the result, and report outputs plus any human-review items.
