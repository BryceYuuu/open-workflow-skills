...
description: "Extract concept hierarchy and relationships from long-form content and export Mermaid, Markmap, or another supported structured format. Use when the user asks for mind map generator or an equivalent workflow."
license: MIT
compatibility: "Cross-agent Agent Skills format. Core stack: LLM / Mermaid or Markmap. Check runtime dependencies before execution."
metadata:
  ...
  search-keywords: "mind-map-generator, Mind Map Generator, 思维导图生成, AI mind map, text to mind map, Markmap, 思维导图, 长文结构化, AI research, learning automation, education AI, 研究助手, 学习自动化, AI教育"
  quality-score: "4.0/5"
---
...
从长文中提取概念层级和关系，并输出 Mermaid、Markmap 等结构化思维导图格式。

**English**

Extract concept hierarchy and relationships from long-form content and export Mermaid, Markmap, or another supported structured format.
...
`mind-map-generator` · `Mind Map Generator` · `思维导图生成` · `AI mind map` · `text to mind map` · `Markmap` · `思维导图` · `长文结构化` · `AI research` · `learning automation` · `education AI` · `研究助手` · `学习自动化` · `AI教育`

## When to use / 何时使用

Use this skill when the user explicitly requests **mind map generator**, or when the requested outcome clearly matches this workflow.
...
```bash
npx skills add BryceYuuu/open-workflow-skills --skill mind-map-generator
```

### Install for a specific supported agent

```bash
npx skills add BryceYuuu/open-workflow-skills --skill mind-map-generator --agent codex
```
...
> 使用 `mind-map-generator` 工作流处理我提供的输入。先检查依赖和权限，不覆盖源文件；执行后给我输出路径、验证结果和需要人工审核的部分。

**English**

> Use the `mind-map-generator` workflow for the provided input. Check dependencies and permissions first, preserve the source, validate the result, and report outputs plus any human-review items.
