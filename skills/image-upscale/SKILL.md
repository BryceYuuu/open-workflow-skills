---
name: image-upscale
description: "Upscale and enhance low-resolution images using a configured local or API model, with a mode that avoids generative detail invention. Use when the user asks for image upscaler or an equivalent workflow."
license: MIT
...
metadata:
  ...
  quality-score: "4.0/5"
---
# Image Upscaler / 图片超分与增强

## Purpose / 用途

**中文**

使用配置好的本地或云端超分模型增强低分辨率图片，并提供尽量避免生成不存在细节的保守模式。

**English**

Upscale and enhance low-resolution images using a configured local or API model, with a mode that avoids generative detail invention.
...
`image-upscale` · `Image Upscaler` · `图片超分与增强` · `AI image upscaler` · `4K upscale` · `super resolution` · `图片超分` · `图片变清晰` · `AI design` · `design automation` · `creative automation` · `AI设计` · `设计自动化` · `创意工作流`

## When to use / 何时使用

Use this skill when the user explicitly requests **image upscaler**, or when the requested outcome clearly matches this workflow.
...
```bash
npx skills add BryceYuuu/open-workflow-skills --skill image-upscale
```

### Install for a specific supported agent

```bash
npx skills add BryceYuuu/open-workflow-skills --skill image-upscale --agent codex
```
...
> 使用 `image-upscale` 工作流处理我提供的输入。先检查依赖和权限，不覆盖源文件；执行后给我输出路径、验证结果和需要人工审核的部分。

**English**

> Use the `image-upscale` workflow for the provided input. Check dependencies and permissions first, preserve the source, validate the result, and report outputs plus any human-review items.
