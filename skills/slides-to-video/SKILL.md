...
description: "Turn slides into narrated training or explainer videos with page timing and basic transitions. Use when the user asks for slides to video or an equivalent workflow."
license: MIT
compatibility: "Cross-agent Agent Skills format. Core stack: Slide parser / TTS / FFmpeg. Check runtime dependencies before execution."
metadata:
  ...
  status: "experimental"
  automation-level: "L3"
  search-keywords: "slides-to-video, Slides to Video, PPT 转讲解视频, PPT to video, PowerPoint to video, slide narration, PPT转视频, 课件转视频, AI content automation, content automation, video automation, social media automation, 内容自动化, 视频自动化"
  quality-score: "3.0/5"
---
# Slides to Video / PPT 转讲解视频

## Purpose / 用途

**中文**

把 PPT/PDF 逐页生成旁白、配音、页面时长和基础转场；不承诺自动生成专业级复杂动画。

**English**

Turn slides into narrated training or explainer videos with page timing and basic transitions.
...
`slides-to-video` · `Slides to Video` · `PPT 转讲解视频` · `PPT to video` · `PowerPoint to video` · `slide narration` · `PPT转视频` · `课件转视频` · `AI content automation` · `content automation` · `video automation` · `social media automation` · `内容自动化` · `视频自动化`

## When to use / 何时使用

Use this skill when the user explicitly requests **slides to video**, or when the requested outcome clearly matches this workflow.
...
```bash
npx skills add BryceYuuu/open-workflow-skills --skill slides-to-video
```

### Install for a specific supported agent

```bash
npx skills add BryceYuuu/open-workflow-skills --skill slides-to-video --agent codex
```
...
> 使用 `slides-to-video` 工作流处理我提供的输入。先检查依赖和权限，不覆盖源文件；执行后给我输出路径、验证结果和需要人工审核的部分。

**English**

> Use the `slides-to-video` workflow for the provided input. Check dependencies and permissions first, preserve the source, validate the result, and report outputs plus any human-review items.
