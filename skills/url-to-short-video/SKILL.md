---
name: url-to-short-video
description: "Turn a product or article URL into a short-video production package with script, shot list, narration, assets, and render candidates. Use when the user asks for url to short video or an equivalent workflow."
license: MIT
compatibility: "Cross-agent Agent Skills format. Core stack: Browser / LLM / TTS / FFmpeg. Check runtime dependencies before execution."
metadata:
  ...
  status: "experimental"
  automation-level: "L3"
  search-keywords: "url-to-short-video, URL to Short Video, 网页转短视频, URL to video, AI short video generator, product video generator, article to video, 网页生成短视频, 链接转视频, AI content automation, content automation, video automation, social media automation, 内容自动化"
  quality-score: "3.0/5"
---
# URL to Short Video / 网页转短视频

## Purpose / 用途

**中文**

读取产品或文章网页，生成脚本、分镜、配音、素材清单和成片候选；最终视觉质量需人工审阅。

**English**

Turn a product or article URL into a short-video production package with script, shot list, narration, assets, and render candidates.
...
`url-to-short-video` · `URL to Short Video` · `网页转短视频` · `URL to video` · `AI short video generator` · `product video generator` · `article to video` · `网页生成短视频` · `链接转视频` · `AI content automation` · `content automation` · `video automation` · `social media automation` · `内容自动化`

## When to use / 何时使用

Use this skill when the user explicitly requests **url to short video**, or when the requested outcome clearly matches this workflow.
...
```bash
npx skills add BryceYuuu/open-workflow-skills --skill url-to-short-video
```

### Install for a specific supported agent

```bash
npx skills add BryceYuuu/open-workflow-skills --skill url-to-short-video --agent codex
```
...
> 使用 `url-to-short-video` 工作流处理我提供的输入。先检查依赖和权限，不覆盖源文件；执行后给我输出路径、验证结果和需要人工审核的部分。

**English**

> Use the `url-to-short-video` workflow for the provided input. Check dependencies and permissions first, preserve the source, validate the result, and report outputs plus any human-review items.
