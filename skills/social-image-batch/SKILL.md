---
name: social-image-batch
description: "Generate batches of social visuals from a brief while keeping naming, aspect ratio, and manifest metadata consistent. Use when the user asks for social image batch or an equivalent workflow."
license: MIT
...
metadata:
  ...
  quality-score: "4.0/5"
---
# Social Image Batch / 社媒配图批量生成

## Purpose / 用途

**中文**

根据内容 Brief 批量生成社交媒体配图，并统一比例、命名和任务清单。

**English**

Generate batches of social visuals from a brief while keeping naming, aspect ratio, and manifest metadata consistent.
...
`social-image-batch` · `Social Image Batch` · `社媒配图批量生成` · `AI image batch generator` · `social media image generator` · `batch image generation` · `批量生图` · `社媒配图` · `AI content automation` · `content automation` · `video automation` · `social media automation` · `内容自动化` · `视频自动化`

## When to use / 何时使用

Use this skill when the user explicitly requests **social image batch**, or when the requested outcome clearly matches this workflow.
...
```bash
npx skills add BryceYuuu/open-workflow-skills --skill social-image-batch
```

### Install for a specific supported agent

```bash
npx skills add BryceYuuu/open-workflow-skills --skill social-image-batch --agent codex
```
...
> 使用 `social-image-batch` 工作流处理我提供的输入。先检查依赖和权限，不覆盖源文件；执行后给我输出路径、验证结果和需要人工审核的部分。

**English**

> Use the `social-image-batch` workflow for the provided input. Check dependencies and permissions first, preserve the source, validate the result, and report outputs plus any human-review items.
