...
description: "Generate audience-specific 30-second, 1-minute, and 3-minute pitches from product and evidence inputs. Use when the user asks for pitch generator or an equivalent workflow."
license: MIT
...
metadata:
  ...
  search-keywords: "pitch-generator, Pitch Generator, Pitch 生成器, AI pitch generator, elevator pitch, startup pitch, 路演稿, 电梯演讲, communication automation, social automation, AI writing assistant, 沟通自动化, 社交媒体自动化, AI写作"
  quality-score: "4.5/5"
---
# Pitch Generator / Pitch 生成器

## Purpose / 用途

**中文**

根据产品、受众和证据输入生成 30 秒、1 分钟和 3 分钟 Pitch。

**English**

Generate audience-specific 30-second, 1-minute, and 3-minute pitches from product and evidence inputs.
...
`pitch-generator` · `Pitch Generator` · `Pitch 生成器` · `AI pitch generator` · `elevator pitch` · `startup pitch` · `路演稿` · `电梯演讲` · `communication automation` · `social automation` · `AI writing assistant` · `沟通自动化` · `社交媒体自动化` · `AI写作`

## When to use / 何时使用

Use this skill when the user explicitly requests **pitch generator**, or when the requested outcome clearly matches this workflow.
...
```bash
npx skills add BryceYuuu/open-workflow-skills --skill pitch-generator
```

### Install for a specific supported agent

```bash
npx skills add BryceYuuu/open-workflow-skills --skill pitch-generator --agent codex
```
...
> 使用 `pitch-generator` 工作流处理我提供的输入。先检查依赖和权限，不覆盖源文件；执行后给我输出路径、验证结果和需要人工审核的部分。

**English**

> Use the `pitch-generator` workflow for the provided input. Check dependencies and permissions first, preserve the source, validate the result, and report outputs plus any human-review items.
