...
description: "Turn verified event facts into a structured press-release draft and produce a fact-check checklist. Use when the user asks for press release generator or an equivalent workflow."
license: MIT
...
metadata:
  ...
  search-keywords: "press-release-generator, Press Release Generator, 新闻稿生成器, AI press release, PR writing, news release generator, 新闻稿生成, 公关稿, communication automation, social automation, AI writing assistant, 沟通自动化, 社交媒体自动化, AI写作"
  quality-score: "4.5/5"
---
# Press Release Generator / 新闻稿生成器

## Purpose / 用途

**中文**

把已核实事件信息整理成结构完整的新闻稿初稿，并生成事实核查 Checklist。

**English**

Turn verified event facts into a structured press-release draft and produce a fact-check checklist.
...
`press-release-generator` · `Press Release Generator` · `新闻稿生成器` · `AI press release` · `PR writing` · `news release generator` · `新闻稿生成` · `公关稿` · `communication automation` · `social automation` · `AI writing assistant` · `沟通自动化` · `社交媒体自动化` · `AI写作`

## When to use / 何时使用

Use this skill when the user explicitly requests **press release generator**, or when the requested outcome clearly matches this workflow.
...
```bash
npx skills add BryceYuuu/open-workflow-skills --skill press-release-generator
```

### Install for a specific supported agent

```bash
npx skills add BryceYuuu/open-workflow-skills --skill press-release-generator --agent codex
```
...
> 使用 `press-release-generator` 工作流处理我提供的输入。先检查依赖和权限，不覆盖源文件；执行后给我输出路径、验证结果和需要人工审核的部分。

**English**

> Use the `press-release-generator` workflow for the provided input. Check dependencies and permissions first, preserve the source, validate the result, and report outputs plus any human-review items.
