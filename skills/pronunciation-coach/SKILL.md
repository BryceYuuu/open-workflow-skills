...
description: "Use a configured pronunciation-assessment engine to evaluate phonemes, stress, rhythm, fluency, and prosody and create targeted drills. Use when the user asks for pronunciation coach or an equivalent workflow."
license: MIT
...
metadata:
  ...
  quality-score: "4.0/5"
---
# Pronunciation Coach / AI 发音教练

## Purpose / 用途

**中文**

调用配置好的发音评测引擎分析音素、重音、节奏、流利度和韵律，并生成针对性练习。

**English**

Use a configured pronunciation-assessment engine to evaluate phonemes, stress, rhythm, fluency, and prosody and create targeted drills.
...
`pronunciation-coach` · `Pronunciation Coach` · `AI 发音教练` · `AI pronunciation coach` · `English pronunciation assessment` · `speaking feedback` · `英语发音` · `口语纠音` · `AI research` · `learning automation` · `education AI` · `研究助手` · `学习自动化` · `AI教育`

## When to use / 何时使用

Use this skill when the user explicitly requests **pronunciation coach**, or when the requested outcome clearly matches this workflow.
...
`Pronunciation assessment API / audio tools`
...
```bash
npx skills add BryceYuuu/open-workflow-skills --skill pronunciation-coach
```

### Install for a specific supported agent

```bash
npx skills add BryceYuuu/open-workflow-skills --skill pronunciation-coach --agent codex
```
...
> 使用 `pronunciation-coach` 工作流处理我提供的输入。先检查依赖和权限，不覆盖源文件；执行后给我输出路径、验证结果和需要人工审核的部分。

**English**

> Use the `pronunciation-coach` workflow for the provided input. Check dependencies and permissions first, preserve the source, validate the result, and report outputs plus any human-review items.
